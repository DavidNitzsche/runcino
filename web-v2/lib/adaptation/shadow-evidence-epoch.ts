...
export const BELIEF_SOURCE_PINS: readonly BeliefSourcePin[] = [
  ...
  },
  {
    ...
    digest: '4ad6154e1221c8ee',
    why: 'Re-pinned ALONE an eighth time, branch (b), 2026-09-15 (BA-01R items 8/9/12, on top of a79c5c86): '
      + '`resolveRaceOutlookBySlug`\'s own computation — `await resolveRaceOutlook(userUuid, race, today)` — is '
      + 'byte-for-byte unchanged; the only edit to its body is capturing that same call\'s result in a local and '
      + 'passing it to a new `cacheProjectionFromOutlook` before returning it, unmodified, to the caller. That '
      + 'new function is a pure side effect (writes a formatted projection string into '
      + '`lib/race/last-known-good-projection.ts`\'s cache, moved out of `plan-snapshot.ts` per item 12) that '
      + 'reads the ALREADY-COMPUTED `RaceOutlook` and feeds nothing back into it. The new exported '
      + '`resolveRaceOutlookCooperative` is likewise additive: a second entry point for `/api/v5/races` that '
      + 'checks a `DeadlineBudget` between the same two I/O phases `resolveRaceOutlookBySlug` already runs '
      + '(`loadRaceForOutlook`, then `loadRaceOutlookReads`) rather than racing the whole call, but calls the '
      + 'exact same `loadRaceForOutlook`/`loadRaceOutlookReads`/`composeRaceOutlook` functions, unmodified, in '
      + 'the same order, with the same arguments. None of `capacity`, `currentProjection`, `expectedRaceDay` or '
      + '`trainingPrescription` — the four resolvers a shadow record compares against — can move for any '
      + 'activity this change touches: nothing about how they are computed changed, only what happens to the '
      + 'finished outlook object afterward (cached, or not, depending on a wall-clock budget that has no path '
      + 'back into the resolution itself). Bumping the epoch would discard the whole shadow corpus for a change '
      + 'that cannot move a single comparison. '
      + 'Re-pinned ALONE a seventh time, branch (b), 2026-09-13 (NATURAL-COACHING-3/4, commit a9006599 on `claude/natural-coaching-programme-lead`, based on `origin/main@a79c5c86`): pure runner-facing copy inside the `controlled_c_effort` limb — the "C race" internal-tier label dropped from `execution.reasonVsExpected` and the bridge\'s `execution_target` step `differsFromPrevious`, and "hard session" reworded to "quality session" for cross-file consistency with `generate.ts`\'s sibling sentence. Verified, not assumed: `composeRaceOutlook` was run before and after the change on identical fixture inputs (`_race_outlook_fixture.ts`) across a 7-goal sweep of the C-race branch, the A/B control branch, and the `execution_target` bridge step. `execution.targetSec`, `.source`, `.effortCharacter`, `.strategyLabel`, `.hr`, and every one of `capacity`, `currentProjection`, `expectedRaceDay`, `trainingPrescription`, `goalFeasibility`, `statedGoal` came out byte-identical in every case (e.g. goal 43:00 → targetSec 2840 both sides; goal 50:00 → targetSec 3000 both sides). The only diffs anywhere in the full object were the two edited prose leaves (`execution.reasonVsExpected`, the bridge step\'s `differsFromPrevious`) and their echo inside that step\'s `evidence` bullet — `resolvedAt` aside, which is a timestamp, not a belief. No number a shadow record compares against moved, so bumping the epoch would discard the whole shadow corpus for a change that cannot move a single comparison. '
      + 'Re-pinned ALONE at epoch 3, branch (b), 2026-09-02 · CEFFORT-1 made `race.priority` load-bearing, so a C race is now priced as a controlled effort (Research/00b §"Recovery by Effort") instead of identically to an A race. What moved is `execution` — the target, the strategy label and the HR band for a C-effort DAY. What did NOT move is every belief a shadow record compares against: `capacity`, `currentProjection`, `expectedRaceDay` and `trainingPrescription` resolve byte-identically for the same activities, and an A or B race is untouched end to end (`_controlled_c_effort.test.ts` asserts that limb explicitly, as the control). A prescription for one day is not a belief about the runner, so no record written under epoch 3 compares against a number this changed, and bumping the epoch would discard the whole shadow corpus for a change that cannot move a single comparison. Stated rather than assumed, per this file\'s own instruction that the decision be made by someone who knows what they changed. '
      ...
      + 'And a fourth time, branch (b), for CEFFORT-2: the controlled-effort share constant moved to '
