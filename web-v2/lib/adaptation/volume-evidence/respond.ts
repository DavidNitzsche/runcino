...
export function respondToVolumeEvidence(input: VolumeResponseInput): VolumeResponse {
  ...
  for (let i = 0; i < ordered.length; i += 1) {
    const prevWeek = i === 0 ? input.weekBeforeFirstFuture : ordered[i - 1];
    if (prevWeek == null) continue;
    const prevMi = proposedByWeek.get(prevWeek.weekStartISO) ?? prevWeek.prescribedMi;
