...
export const AUTOMATIC_MUTATIONS: readonly AutomaticMutation[] = [
  // ── The plan writers ──────────────────────────────────────────────────────
  {
    ...
    note:
      'ONE TIER since 2026-09-02. DRIFT NO LONGER PROPOSES ANYTHING. Soft drift (volume/vdot/staleness/'
      + 'easy/long/quality) and the goal-gap widening trend used to write a pending plan_proposals row whose '
      + 'accept re-authored the block; both writers are deleted, and a card of one of those kinds left '
      + 'pending in a live account is REFUSED on accept server-side (RETIRED_REBUILD_PROPOSAL_KINDS in '
      + 'lib/plan/drift-proposal-policy.ts, checked in POST /api/plan/proposal). detectDrift still runs and '
      + 'its counts are still reported: observational only, no plan row, no card. Owner 2026-09-02 — "too '
      + 'many independent levers can soften, reshape, re-phase, refuse, or automatically mutate the plan" — '
      + 'and every drift trigger is a transient reading rather than an authored fact. What survives writes '
      + 'a plan only on the CALENDAR: lifecycle transitions AUTO-APPLY '
      ...
      + '(runnerIsCompromised: training-gap re-entry — the illness / injury / override-niggle arms were '
      + 'deleted 2026-09-02 when the runner took ownership of readiness — or an elapsed injury-return '
  ...
  },
  {
    ...
    note:
      'SEALED 2026-09-02 · THIS CRON NO LONGER CHANGES THE PLAN. Every action it produces goes through '
      + 'sealAutomaticActions (lib/plan/adaptation-authority.ts — THE ONE SEAM, default off) and comes out '
      + 'as a runner-gated plan_workout_proposals card or as a record-only coach_intents note under the '
      + 'reason plan_adapt_sealed. tryAdaptiveBump refuses at the same seam before it reads anything, so '
      + 'Rule 21\'s volume axis is shadow-only. plan_workouts REMAINS in `changes` above, and deliberately: '
      + 'the registry keys a writer on the FUNCTION this route invokes, applyAdaptations still contains '
      + 'those UPDATE statements, and they are still reached — by the runner tapping accept on '
      + 'POST /api/plan/workout-proposals/[id]/accept, and by this cron again the moment the seam is '
      + 'opened. Listing the table the invoked function can write is the honest answer; claiming the '
      + 'capability had vanished would be the dishonest one. Historical behaviour below. '
