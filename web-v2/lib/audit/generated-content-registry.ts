...
export const MODULE_ORPHANS: Record<string, string> = {
  ...
  'web-v2/lib/training/pace-corpus.ts':
    'DELIBERATELY UNWIRED — Phase 2 of the owner\'s 2026-08-30 anchor-in-the-running ruling (see vdot-corpus.ts, already wired, for Phase 1). Two direct-evidence readers, resolveEasyPaceCorpus/resolveThresholdPaceCorpus, that read easy and threshold pace straight from classified training instead of through the VDOT/Daniels-table cascade — built, unit-tested, doctrine-cited, and rendered against the owner\'s real account (its own .audit.test.ts). Wiring them into resolveCurrentTPace/generate.ts/spec-builder.ts is Phase 3, explicitly deferred to its own focused pass because it touches the plan engine\'s core pace derivation, the archetype sweep and the doctrine registry all at once and deserves full re-verification on its own. Should be WIRED, not deleted, once that pass lands.',
  'web-v2/lib/training/durability-anchor.ts':
    'DELIBERATELY UNWIRED — the DURABILITY anchor of the fitness-vector rebuild the owner ordered 2026-08-30/31, same posture as pace-corpus.ts above. resolveRaceExponent (a personal Riegel exponent fitted from the runner\'s own race history, shrunk toward the population endurance prior by evidence quality) and resolveDecoupling (corroborated aerobic-decoupling across qualifying long runs) — built, unit-tested, doctrine-cited, and rendered against the owner\'s real account (its own .audit.test.ts: his fitted exponent reads ~1.10 against the population prior of 1.06). Wiring into resolveCurrentTPace, generate.ts or race-prediction is a later pass, explicitly out of scope for this one — a fresh reader with no caller is exactly this gate\'s "fine reason" case, not a bug. Should be WIRED, not deleted, once that pass lands.',
  'web-v2/lib/training/capacity-resolver.ts':
    'DELIBERATELY UNWIRED — the RUNNER MODEL OWNERSHIP LAYER, the phase that sits on top of pace-corpus.ts and durability-anchor.ts above and gives all three the single owner docs/DOCTRINE_ENFORCEMENT_AND_CLEAN_IMPLEMENTATION.md §1-2 requires: one owning service for "what do we currently believe about this runner\'s underlying capacity", and exactly one canonical resolver per derived value. Four of them — resolveThresholdCapacity, resolveHighIntensityCapacity, resolveDurability, resolveEasyCeiling — each carrying the §16 fallback ladder INSIDE itself, a §17 source_mode, a 0..1 confidence on three non-overlapping bands, and traceable evidence ids. Built, doctrine-tested (goal isolation enforced at COMPILE TIME, ladder falsified in both directions, confidence monotonicity as property tests), doctrine-cited (CONVENTION.capacity-confidence-bands), and rendered against the owner\'s real account in its own .audit.test.ts — where it found and fixed a live monotonicity defect in the durability aggregate. Wiring it into generate.ts / spec-builder.ts / reanchor-plan.ts / recompute-paces.ts and retiring the direct VDOT call sites underneath it is the NEXT phase, explicitly scoped separately for the same reason the two readers below were: it touches the plan engine\'s core pace derivation, the archetype sweep and the doctrine registry at once and deserves full re-verification on its own. Should be WIRED, not deleted, once that pass lands.',
  'web-v2/lib/training/runner-state.ts':
    'DELIBERATELY UNWIRED — the READINESS/STATE OWNERSHIP LAYER, the state half of the fitness-vector rebuild the owner ordered 2026-08-30/31, same posture as capacity-resolver.ts above. `resolveRunnerState` consolidates machinery that is ALREADY WIRED and already gated — gradeConvergence (five domains, CONVERGENCE.* registry claims), runnerIsCompromised, computeAcwr, the post-race window — into the one typed answer BRIEF 08 specifies: proceed / proceed_with_caution / reduce / replace / recover / stop, with a Rule 11 `readable` flag so a failed read is never mistaken for a healthy runner. It INVENTS NO READINESS RULE and detects nothing new; `readiness_pullback` in lib/plan/adapt.ts still owns every mutation, untouched. Built, unit-tested (precedence, non-driving signals, the unreadable case), goal-isolated at COMPILE TIME, and rendered against the owner\'s real account in lib/training/_prescription_resolver.audit.test.ts. Wiring it is the same NEXT phase as capacity-resolver.ts and prescription-resolver.ts; docs/DOCTRINE_ENFORCEMENT_AND_CLEAN_IMPLEMENTATION.md §21 requires exactly this shadow-mode step before a new resolver is given authority. Should be WIRED, not deleted, once that pass lands.',
  'web-v2/lib/training/prescription-resolver.ts':
    'DELIBERATELY UNWIRED — the PACE PRESCRIPTION OWNERSHIP LAYER, the phase that sits on top of capacity-resolver.ts and runner-state.ts and answers §1\'s "given current capacity and workout purpose, what intensity should be prescribed". One pure function, `resolvePrescription`, routing BRIEF 03\'s table: easy/long/shakeout to the easy CEILING (never a band — doctrine §9 and the 2026-08-31 decision), threshold to threshold capacity, intervals and reps to high-intensity capacity, and marathon-specific to threshold capacity carried out to 26.2 through the runner\'s OWN fitted endurance exponent instead of the flat population T+18. §7 is enforced by the COMPILER — the capacity input is Immutable<>, so a state adjustment writing back onto a capacity estimate does not typecheck — and §6 by a sealed argument key-set; both were falsified before landing. Race day is DECLINED rather than answered, because it is the one prescription that legitimately reads a stated goal and §1 gives it to Race Prediction. Doctrine-tested (goal poisoning, fatigue, low-confidence widening, §30 monotonicity, a Rule 9 continuity walk, and a falsifiable check that its Riegel step agrees with the app\'s own predictRaceTimeFromAnchor at the population exponent), and rendered against the owner\'s entire live 103-row CIM block in its own .audit.test.ts, which prints the per-day disagreement report §21 asks for. Wiring it into generate.ts / spec-builder.ts and retiring the pace derivations underneath is the NEXT phase, scoped separately for the same reason capacity-resolver.ts was. Should be WIRED, not deleted, once that pass lands.',
  'web-v2/lib/evidence/activity-evidence.ts':
    'DELIBERATELY UNWIRED — the EVIDENCE ENGINE OWNERSHIP LAYER, the piece docs/DOCTRINE_ENFORCEMENT_AND_CLEAN_IMPLEMENTATION.md \u00a78\u2019s pipeline was missing between a raw activity and the capacity resolvers, same posture as capacity-resolver.ts and prescription-resolver.ts above. `classifyActivityEvidence` answers \u00a71\u2019s "what did this completed activity demonstrate" for ONE activity in isolation: BRIEF 02\u2019s two stages (per-SIGNAL eligibility, then weight), crosswalk/interruption detection before any drift read, segment inference on an unlabelled run, and a per-capacity evidence/no-evidence/indeterminate union whose no-value arms carry no weight field so Rule 11 is a type error. It calls no resolver and decides no adaptation — both are other layers\u2019 questions and the file header states the boundary. Graded against the two locked reference cases in docs/reference-cases/ by _activity_evidence.test.ts, and RENDERED against the owner\u2019s two real production rows by _activity_evidence.audit.test.ts. Wiring it (the ingest path calling it, and the Runner Model consuming its ledger entries and its CONTRADICTS_CURRENT_ESTIMATE signal to lower a future corroboration bar) is the NEXT phase, scoped separately for the same reason the resolvers above were. Should be WIRED, not deleted, once that pass lands.',
  'web-v2/lib/adaptation/adaptation-engine.ts':
    'DELIBERATELY UNWIRED — the ADAPTATION ENGINE OWNERSHIP LAYER, the layer that sits on top of capacity-resolver.ts, runner-state.ts and activity-evidence.ts and answers docs/DOCTRINE_ENFORCEMENT_AND_CLEAN_IMPLEMENTATION.md §1’s "what should change in response to new evidence", per BRIEF 07 and docs/ADAPTATION_PROGRESSION_DOCTRINE.md. Same posture as the four layers above it. `composeAdaptation` returns §9’s AdaptationProposal as a DISCRIMINATED UNION ON THE LEVER, so a schedule restructure cannot carry a pace magnitude and a compound "change pace AND density AND duration" proposal is not expressible — one primary stressor at a time is a property of the type. Four INDEPENDENT levers (pace from capacity evidence WITH CONTROL, volume from load tolerance, duration from long-run tolerance, density carried whole from resolveWeekProgression), each detector receiving only its own slice of the input so BRIEF 07’s "adapt the thing that changed" is plumbing rather than discipline. It CONSOLIDATES rather than duplicates: classifyAdaptation is its absorption input, resolveWeekProgression owns density, adaptive-ramp.ts owns the volume/duration caps, pace-anchor.ts owns the pace quantum, and runner-state.ts owns readiness — nothing here re-derives any of them. Goal isolation and capacity immutability are enforced at COMPILE TIME; the one-stressor rule, the HOLD-does-not-move-the-number rule and safety precedence are enforced by a contradiction checker that was falsified in all three directions before landing. Doctrine-tested (goal poisoning, single-run resistance, the Example-A-vs-Example-B control test, one-stressor-at-a-time, all four decisions proven reachable), doctrine-cited (CONVENTION.adaptation-engine-bars), and RENDERED against the owner’s real account in its own .audit.test.ts — where it caught a live defect (two LOAD-domain levers disagreeing about the same absorption verdict) and where a Rule 21 probe walks his whole season to show each upward bar is a bar rather than a wall. Wiring it into the run-adaptations cron and retiring the overlapping detectors in lib/plan/adapt.ts is the NEXT phase, scoped separately for the same reason every layer below it was: it touches the mutation path, the cron and the doctrine registry at once. Should be WIRED, not deleted, once that pass lands.',
  ...
  'web-v2/lib/evidence/reexamination.ts':
    'THE BELIEF-TENSION CONSUMER, and the follow-up activity-evidence.ts named in its own header when it said "NOT CONSUMED YET". It accumulates the CONTRADICTS_CURRENT_ESTIMATE signal across recent activities and lowers the corroboration floor for the NEXT confirming observation by at most ONE, never below two — it cannot move an estimate, cannot raise a bar, and cannot make one activity sufficient to anchor a capacity. Placed in the RUNNER MODEL layer rather than the Adaptation Engine on purpose: which evidence counts toward a belief is §1’s Runner Model question, and an Adaptation Engine that relaxed the bar itself would be forming a belief the Runner Model did not form. It IS consumed — composeThresholdCapacity takes it through the minObservations seam it already exposed, and resolveThresholdCapacity loads it in a second pass — but the whole capacity-resolver.ts chain is itself deliberately unwired, so this module inherits that status and dies or lives with it. Rule 9 compliance is the part worth reading: the relaxation is gated on a COUNT of discrete facts the Evidence Engine already graded, never on a threshold over the continuous reexamination weight, and a walk across the entire weight range asserting one constant outcome is the falsifier for that.',
  'web-v2/lib/evidence/load-activity-evidence.ts':
    'The database shell for activity-evidence.ts above, and unwired with it. It holds no judgement — it reads one canonical `runs` row, the ACTIVE plan\u2019s day, and the runner\u2019s real post_run_rpe/subjective_checkins, normalises them through the shared readers (runFacts, normalizeSplits, runCadenceSpm) and calls the pure classifier. Split out for the same reason capacity-resolver.ts splits its DB shells: every judgement stays testable without a database. Dies or lives with activity-evidence.ts.',
