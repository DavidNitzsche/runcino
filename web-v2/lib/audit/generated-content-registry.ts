...
export const MODULE_ORPHANS: Record<string, string> = {
  'web-v2/lib/evidence/classify-evidence.ts':
    'EVIDENCECLASSIFIER-1 (2026-09-05). THE CANONICAL EVIDENCE CLASSIFIER, CLAUDE.md\'s overnight priority 5, and deliberately not wired into a production route in this change. It composes execution identity (day-resolver.ts), duplicate/merge role (identity.ts\'s CANONICAL_ROW_SQL predicate), race authority (effort-authority.ts), terrain and heat (run-terrain.ts, heat-model.ts), HR-trace credibility (hr-trace-credibility.ts) and safety state (load-safety.ts) into ONE tagged record per activity, so a future caller does not have to re-derive any of those from scratch. It has no runtime caller yet for the same reason lib/runner-state/belief.ts above has none: the consumer is a separate agent\'s work on lib/runner-state/ (the belief store), landing the same night, and this file\'s job was to publish a clean, composable shape for that consumer to read once it exists — not to force a premature wiring decision into a route this change did not audit. It is exercised directly by _classify_evidence.test.ts (22 cases, three of them Rule-18 falsifications planted and watched to fail before being restored) against its pure core, buildEvidenceClassification, which takes no I/O at all. THIS ENTRY EXPIRES the moment anything under app/ or lib/runner-state/ imports classifyEvidence or buildEvidenceClassification, at which point the wiring has to be argued on its own rather than inheriting this exemption.',
  ...
  'web-v2/lib/runner-state/belief.ts':
    'RUNNERSTATE-1 (2026-09-05). THE SHAPE OF ONE RUNNER BELIEF, AND DELIBERATELY NOT A RESOLVER. David: "The brain needs a coherent model of the runner, not separate facts interpreted differently by each feature." This file defines what a belief CARRIES (value or range, confidence, supporting AND contradicting evidence, recency, provenance, last update, what would move it up, what would move it down, what must never move it) and computes not one physiological number. It has no caller because the LOADER that fills it belongs in lib/brain/, which is a separate owner and does not exist yet; this side is the contract that loader has to satisfy. It is PURE: every import is `import type`, so it emits no import statement at all and reaches no database at any depth, and _runner_state.test.ts walks the transitive graph and fails if that ever stops being true (Rule 19 was earned by a header that asserted exactly this claim and was false for a day). Rule 11 is a TYPE here, not a discipline: the reading is `Measured<T>` type-imported from lib/adaptation/canonical/input rather than a fifth spelling of the same union, and its refusal branch carries no `value` field. THIS ENTRY EXPIRES the moment lib/brain/ imports it, at which point the loader has to be argued against the ownership registry rather than inheriting this exemption. Deleting it instead is a fine outcome; what is not fine is it acquiring a caller quietly.',
  'web-v2/lib/runner-state/quantity-owners.ts':
    'OWNER-AGREEMENT-1 (2026-09-05). A GATE\'s registry, in the same posture as lib/brain/proposal/facets.ts and lib/doctrine/runner-facing-violations.ts: it is DATA FOR A CHECK and its only importer is _owner_agreement.test.ts by design. It names, for each of the twelve quantities the owner listed (weekly volume, long-run distance, threshold dose, interval pace, interval dose, marathon-pace dose, quality frequency, running frequency, recovery spacing, race target, goal, heat and terrain response), which site is THE answer, which other production-reachable sites also answer it, and — this is the part ownership.ts beside it cannot do — the MEASURED delta between them for one runner on one date. Runtime code must never import it: it records what DISAGREES, not a value anything should render, and a runtime consumer would start branching on the divergences. It also holds no engine import at all, deliberately, because importing lib/plan/spec-builder or lib/training/capacity-resolver would pull `pg` into lib/runner-state/ and _runner_state.test.ts fails the whole directory on a runtime import; the resolvers that actually CALL each site live in the gate. THIS ENTRY EXPIRES if anything under app/ or lib/brain/ imports it, at which point a registry of open defects has become a runtime input and has to be argued rather than inheriting this exemption. Added with scripts/check-belief-owners.sh.',
  'web-v2/lib/runner-state/ownership.ts':
    'Same directory, same exemption: the OWNERSHIP MAP. For each of the twenty runner facts, which exported symbol is THE answer, which other symbols also answer it, and whether they can disagree. It is docs/BRAIN_CONSTITUTION.md section 29 applied one level down, at the level of a function rather than a domain, and it exists because section 5 forbids exactly what the survey found: eighteen of the twenty beliefs have more than one live answer, four have no canonical owner at all, and five separate formulas answer "what is this runner resting heart rate baseline". Nothing here is a fix; every conflict is recorded with both file references and a stated verdict, and _runner_state.test.ts resolves every module and symbol against the real file so a rename fails the build instead of leaving a map pointing at nothing. It declares no numeric constant and the suite scans for one, because a physiological number appearing in an ownership map means a second brain has started.',
  'web-v2/lib/runner-state/assemble.ts':
    'Same directory, same exemption: the pure assembler, and the ONE entry point lib/brain/ is meant to call. It takes answers the canonical owners have already produced and gives them one shape; it performs no query, no clock read and no arithmetic. The load-bearing decision is what a caller may NOT supply: a loader submits a reading, a confidence, a provenance and its evidence, and the Rule 8 side, the owner reference and the three lever lists are welded on from the registry, so a loader cannot declare its own number filtered, cannot name itself the owner of a belief it does not own, and cannot drop the prohibition that says a stated goal must never move a capacity. The input type is TOTAL over all twenty keys, so a loader that did not look has to say so rather than leaving a hole that reads as a null downstream, which is the collapse lib/topics/types.ts CoachState instructs every consumer to make. Orphaned for the same reason as belief.ts and expiring on the same condition.',
  'web-v2/lib/runner-state/store/schema.ts':
    'BELIEFSTORE-1 (2026-09-06). THE DURABLE SCHEMA for `runner_beliefs` — steps 1 ("load canonical runner state") and 5 ("update beliefs", numbered 7 in the overnight brief that commissioned this) of lib/brain/orchestration/steps.ts, both UNWIRED because "there is no belief store." This directory is that store, built and proven against the local scratch database `faff_roundtrip_scratch` (see lib/runner-state/store/_belief_store.db.test.ts — schema creation, an append-only write path, a real archive-and-replace rebuild proving `plan_lineage_id` survives it, and three cold-start postures — refuse, low-confidence fallback, measured zero — all falsified against the unfixed code first). It is deliberately NOT migrated onto production tonight: creating this table there needs a reviewed migration under this repo\'s DDL-needs-a-per-statement-go doctrine, which is a decision for the owner, not a belief-store wiring pass. THIS ENTRY EXPIRES the moment a reviewed migration lands `runner_beliefs` on production AND a route or cron under app/ imports `updateRunnerBeliefs` / `loadRunnerBeliefs` — at that point WIRED_STEP_PIN in steps.ts has to rise and be argued, rather than this exemption being inherited quietly.',
  'web-v2/lib/runner-state/store/record.ts':
    'Same directory, same exemption: the one (de)serialization bridge between a `Belief<T>` and a `runner_beliefs` row, so no other file in this directory has to decide twice what `reading_ok = false` means on the way back out. Pure — no query, no clock read.',
  'web-v2/lib/runner-state/store/lineage.ts':
    'Same directory, same exemption: resolves which `plan_lineage_id` a belief update should carry, by calling lib/brain/ledger/decision-ledger.ts#resolvePlanLineage against the runner\'s active and most-recently-archived plan rather than re-deriving the chase. This is the file the rebuild-survival proof is actually about.',
  'web-v2/lib/runner-state/store/write.ts':
    'Same directory, same exemption: the one INSERT into `runner_beliefs`. Append-only — there is no UPDATE anywhere in this directory, matching plan_decision_ledger\'s own posture on rows that must never be quietly overwritten.',
  'web-v2/lib/runner-state/store/read.ts':
    'Same directory, same exemption: defines "the current belief" as the newest row per key, and applies the STORE\'s OWN staleness discount at read time — a second, separate decay from lib/adaptation/volume-evidence/weight.ts#recencyWeight and lib/training/durability-anchor.ts#recencyWeight, which age evidence INSIDE a resolver; this one ages a CACHED ROW that was never refreshed, and is falsified in _belief_store.test.ts by planting a no-decay bug and watching the suite name it.',
  'web-v2/lib/runner-state/store/loaders.ts':
    'Same directory, same exemption: the twelve belief loaders the overnight brief asked for, each calling the ONE registered owner ownership.ts or quantity-owners.ts already names for that belief (sustainedWeeklyMileage, computeAcwr, resolveThresholdCapacity, resolveHighIntensityCapacity, marathonPaceFromDurability, resolveSafety, composeRaceOutlook) — never a second computation. Two beliefs (MAX_DEMONSTRATED_DOSE, TRAINING_PHASE) have no canonical owner at all per ownership.ts\'s own OPEN findings and are submitted as honest refusals naming that finding, not manufactured. One belief (LONG_RUN_TOLERANCE) has a registered PURE owner (lib/plan/generate.ts#evidenceLongCeilingMi) whose real inputs are private closures with no exported reader, and is submitted as a refusal naming that seam rather than a duplicated reading. The eight beliefs outside this session\'s scope are `notLookedFor`, Rule 11\'s third fact.',
  'web-v2/lib/runner-state/store/quantity-loaders.ts':
    'Same directory, same exemption: the three dose ceilings (THRESHOLD_DOSE, INTERVAL_DOSE, MARATHON_PACE_DOSE), derived from the weekly-volume belief and the registry\'s own OWNER functions (atPaceSessionCapMi, MARATHON_PACE_WORKOUT_CAP) rather than a thirteenth computation — these are prescription ceilings, not runner facts, so they are stored under the QUANTITY registry rather than inventing a 21st BeliefKey. LONG_RUN_DISTANCE\'s share-of-week half is deliberately NOT here: its OWNER sits behind the walled canonical adaptation engine\'s import allowlist (_zero_mutation_scan.test.ts guard 3, _cannot_mutate.test.ts / _never_mutates_plan.test.ts guard 4), which this directory is not on, and its two SECOND answers disagree with the OWNER — spending either would be the thirteenth-owner defect this store exists to avoid. Named in this file\'s own header rather than faked.',
  'web-v2/lib/runner-state/store/orchestrator.ts':
    'Same directory, same exemption: `updateRunnerBeliefs` (step 5/"7") and `loadRunnerBeliefs` (step 1), as two callable functions rather than a route. Not wired to app/ tonight for the reason schema.ts\'s entry states — the table has no production migration yet, which is a decision this session does not get to make for the owner.',
  'web-v2/lib/plan/adjudication/false-block.ts':
    'COLDSTART-PROMO-1 (2026-09-05). THE FALSE-BLOCK CLASSIFIER, and a GATE\'s helper rather than a runtime module — the same posture as lib/doctrine/runner-facing-violations.ts and lib/brain/proposal/facets.ts above: it records what a REFUSAL is worth, not a value anything should render, and a runtime consumer would start unblocking plans with it. It answers one question about a sentence checkPromotion already emitted: is this a defect in the plan, or an artefact of the reading? Two shapes are decidable — a progression block on a runner whose every PUSH option was unrankable (nothing to advance FROM, which is what refused six of seven active plans before the cold-start policy), and a wholeBlockCoherence block on a plan with no future weeks (finished, not incoherent, which is the seventh). It UNBLOCKS NOTHING: both sentences still block, because softening either reopens the empty-set hole Rule 18 exists to own. It started as a local function inside _promotion_replay.script.ts and was moved out precisely because a classifier only exercisable against production is one nothing in CI can falsify; _false_block.test.ts drives it from the REAL gate\'s output rather than typed literals, so a change to either sentence breaks the test instead of silently retiring the classifier, and it carries three UNCLASSIFIED cases against three FALSE ones so a classifier that excused everything would fail. THIS ENTRY EXPIRES the moment anything under app/ or a cron imports it: at that point a refusal is being acted on rather than reported, and the decision to route a plan past a gate has to be argued on its own rather than inheriting this exemption.',
