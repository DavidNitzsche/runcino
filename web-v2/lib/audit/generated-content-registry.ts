...
export const MODULE_ORPHANS: Record<string, string> = {
  ...
  'web-v2/lib/plan/authoring-shadow-compare.ts':
    'TEST-ONLY BY DESIGN, not a gap waiting to be wired. Scoped by docs/reports/canonical-authoring-migration-2026-09-01.md: `generate.ts` still authors every real plan through the legacy VDOT cascade, and this file exists to compare that legacy path against the canonical `capacity-resolver.ts`/`prescription-resolver.ts` layer WITHOUT changing what any real caller persists — the report\'s explicit boundary is "do NOT switch initial authoring over to the canonical path" until a human has reviewed the diffs this file produces. `canonicalSpecForComposedDay` is a shadow twin of `generate.ts`\'s own `specForComposedDay`, reachable only from `_authoring_shadow_compare.audit.test.ts` (real accounts) and `_authoring_shadow_compare.test.ts` (pure). If a future pass promotes the canonical path to authoring authority, this file is the place that migration starts from — until then, a runtime importer here would be the exact defect the report was written to avoid.',
