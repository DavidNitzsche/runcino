...
export const MODULE_ORPHANS: Record<string, string> = {
  ...
  'web-v2/lib/plan/_render_evidence_duration_offer.script.ts':
    'ROUND3-FIX (2026-09-12). A scratch script (not a .test.ts the default vitest include glob picks up, deliberately), same posture as the other `.script.ts` diagnostics registered above it: run once by hand via `npx tsx lib/plan/_render_evidence_duration_offer.script.ts`, not part of any suite, and produces no report file. It exists per CLAUDE.md Rule 13 -- "a fix to something the runner sees is verified by RENDERING it" -- for DURATIONOFFER-1/2: it calls the real firstDurationAccelerate/actionHeadline production functions with representative ProgressionResolution shapes (a genuine strong-evidence ACCELERATE, a resumed TAKE, and a BACK_OFF) and prints the actual structural strings a DURATION_PROGRESS_OFFER card would carry to stdout as JSON, so the ruling could be checked against real generated prose rather than a hand-typed HTML mockup. This entry was missing from the branch\'s own final commit, which is what this gate caught. Runtime code must never import it.',
  ...
  'web-v2/lib/audit/race-week-canonical-registry.ts':
    'A GATE, same shape as active-plan-exemptions.ts / normal-window-registry.ts: RACEWEEK-CONSOLIDATION-1\'s inventory of every raw read of plan_weeks.is_race_week / an isRaceWeek-named field, with an EXACT pinned count and an argued reason per file (CANONICAL_DEFINITION / CANONICAL_CONSUMER / GOAL_ONLY_DELIBERATE / STRUCTURAL_NONREAD / WEB_FRONTEND_OUT_OF_SCOPE / FOLLOWUP). Read only by lib/audit/_race_week_canonical_scan.test.ts and scripts/check-race-week-canonical-reads.sh. Runtime code must never import it — the point is that a reader either resolves through weekContainsRace/isGoalRaceWeek/resolveRaceWeekRole/containsRaceOf or is listed here, and nothing should be able to consult the list to decide a coaching question. The list is a ratchet in both directions: the scanner\'s exact-count assertion fails on a new unreviewed read AND on a stale pin whose site was fixed or removed, so it cannot rot in place.',
  ...
  'web-v2/lib/training/training-consistency.ts':
    'F077 (2026-09-14/15). THE CANONICAL `TRAINING_CONSISTENCY` READER (see `lib/runner-state/ownership.ts`\'s entry of that name for the full consolidation record) — `resolveTrainingConsistency` / `loadTrainingConsistency`, answering whether the same quality-session stimulus has kept failing to land as a trailing consecutive-miss streak, per the coaching consult\'s doctrine determination for IPR-20260914-006. Built and tested this pass (Phase 1 of the F077 work, plus Phase 2 item 4, the pure detection primitive in `lib/coach/non-adherence-offer.ts` beside it) but NOT wired to a real production surface yet — that is Phase 2 item 5, and it needs a new offer-history persistence layer (this project\'s own standing rule: DDL needs its own per-statement approval) plus a dedicated look at the existing HOLD/RECORD_ONLY coach-decision-card mechanism before it can be reached from a cron or a route. Its only callers today are its own test file and (transitively, by citation only, not by import) `non-adherence-offer.ts`\'s doc comments. Full scope boundary in `programme-internal-working/00-master-programme/F077-TRAINING-CONSISTENCY-2026-09-14.md`. THIS ENTRY EXPIRES the moment a cron or route imports it — at that point note here which surface adopted it.',
  'web-v2/lib/coach/non-adherence-offer.ts':
    'F077 (2026-09-14/15), Phase 2 item 4. `resolveNonAdherenceOffer` — decides which of RECOMMIT/RECALIBRATE the coach should lead with once `lib/training/training-consistency.ts#resolveTrainingConsistency` has triggered, per consult-log 2026-09-15-025 Ruling 2 (the recommit-cycle-of-2 escalation). Pure and fully tested (Rule 18: genuine first ask, escalation after a prior RECOMMIT, staying escalated on recurrence, a lone prior RECALIBRATE not forcing escalation), but NOT wired to a real coach-decision-card surface yet — that needs an offer-history table this pass did not have standing to add (DDL requires its own approval) and a dedicated read of the HOLD/RECORD_ONLY pattern it should reuse rather than inventing a new one. Its only caller today is its own test file. Full scope boundary in `programme-internal-working/00-master-programme/F077-TRAINING-CONSISTENCY-2026-09-14.md`. THIS ENTRY EXPIRES the moment a real coach surface imports it.',
};
