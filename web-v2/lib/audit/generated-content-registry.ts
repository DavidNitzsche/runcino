...
export const MODULE_ORPHANS: Record<string, string> = {
  ...
  'web-v2/lib/adaptation/volume-evidence/weight.ts':
    'CONTINUOUS-EVIDENCE-1 (2026-09-05). Same directory, same shadow-only exemption, and the file that removed the cliff the owner rejected: "The closest historical week completed 47.3 against 45.5 prescribed but contributed zero evidence because it missed a 47.8 bar by 0.4 miles. That is another cliff." It holds the curves and the COEFFICIENTS ledger -- every constant with a provenance of CALCULATED_PHYSIOLOGY (read out of a Research/ doc at gate time), ATHLETE_EVIDENCE (measured, and nothing is yet) or POLICY_ASSUMPTION (chosen, and it must say so in its own text). The GPS noise gate, the per-week credit ceiling, the progression unlock, the absorption ramp and the recency ramp all live here as pure functions with no clock, no database and no plan read. Three DOCTRINE_REGISTRY claims watch the cited half (VOLUME.evidence-noise-floor-is-the-gps-error-band, VOLUME.credit-ceiling-and-unlock-are-the-cycle-growth-band, VOLUME.evidence-window-is-the-chronic-load-window) and _continuous_evidence.test.ts watches the chosen half. See contract.ts for why the directory has no runtime caller.',
  'web-v2/lib/adaptation/volume-evidence/evidence.ts':
    'CONTINUOUS-EVIDENCE-1 (2026-09-05). Same directory, same shadow-only exemption. THE SPINE ENTRY POINT: readWeekEvidence takes one week of executions and returns TWO channels that are never collapsed into one number -- capacity (Rule 8 FILTERED, what the runner has shown he can carry) and fatigue (Rule 8\'s COROLLARY, deliberately unfiltered, what the legs actually absorbed). The owner\'s hard requirement is the reason they are separate types on separate fields: "Extra mileage during a recovery week may increase fatigue, affect the next decision, and fail to earn a future mileage increase. A duplicate or recording artifact must affect neither." It also holds accumulateCapacityEvidence, the ledger that lets repeated modest overruns add up, and it is the function a lib/brain/ spine is expected to call. Pure: no pool, no fetch, no clock. See contract.ts for why the directory has no runtime caller.',
  ...
  'web-v2/lib/brain/proposal/facets.ts':
    'A GATE\'s registry, in the same posture as lib/doctrine/runner-facing-violations.ts and lib/audit/coercion-registry.ts immediately below: it is DATA FOR A CHECK, and its only importer is _action_completeness.test.ts by design. It holds the eleven facets a BrainAction kind owes before it is real (generator, validator, serializer, renderer, explanation, accept executor, mutation, ledger, undo, watch, integration test), the GENERATOR_REGISTRY naming which module emits each kind and which live path reaches it, and the FACET_GAPS ratchet naming every one of the 231 cells that is genuinely absent with an argued reason. Runtime code must never import it: it records what is MISSING, not a value anything should render, and a runtime consumer would start branching on the gaps. The gate cross-checks it against real code in both directions — a gap standing while executorFor stops saying UNIMPLEMENTED fails, and so does the reverse — so it cannot drift into a stale list the way an ordinary allowlist can. Added 2026-09-05 with scripts/check-action-completeness.sh.',
  'web-v2/lib/brain/proposal/generate/from-safety.ts':
    'ACTIONCOMPLETE-1 (2026-09-05). THE SAFETY_STOP GENERATOR, COMPLETE AND DELIBERATELY UNWIRED, in the same posture as lib/plan/adjudication/dose-responsive.ts above: built, driven end to end by its gate, and waiting on a caller that is another slice\'s to write. It takes a SafetyResolution as an INPUT and never re-derives one, because docs/BRAIN_CONSTITUTION.md gives Safety exactly one canonical owner (lib/safety/**) and lib/watch/safety-stop.ts already had to be corrected once for re-deriving a predicate that owner exports. It is PURE: no database, no clock, and its only non-type import is safetyVerdictLine from the safety owner itself, so the sentence on the card, the watch board and the ledger are one sentence (Rule 16). WHY IT IS NOT WIRED YET: the canonical safety state (injury, illness, niggle into lib/plan/adjudication/live-input.ts) is in flight in a separate slice, and editing those sources from here would create the second answer the constitution forbids. The two halves meet at safetyStopFrom({ resolution }) with nothing left to negotiate. Its behaviour is asserted in three directions by lib/brain/proposal/_action_completeness.test.ts — an UNKNOWN verdict yields null rather than a stop (turning a failed check into a stop would tell a healthy runner to stop because the database blinked), a MODIFY yields null (easy-only is a different prescription, not a halt), and a STOP yields a SAFETY_STOP whose sentence comes from the safety owner. THIS ENTRY EXPIRES the moment a live path calls it, at which point the GENERATOR gap for SAFETY_STOP in lib/brain/proposal/facets.ts has to be deleted in the same change or the completeness gate fails on the stale entry.',
