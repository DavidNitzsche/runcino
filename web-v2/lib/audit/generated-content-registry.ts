...
export const MODULE_ORPHANS: Record<string, string> = {
  ...
  'web-v2/lib/plan/adjudication/adjudicate.ts':
    'Same layer, the reasoning half. Pure functions over already-loaded facts -- it opens no database and reads no plan, so it can be tested against constructed sequences. Read only by _adjudication.test.ts (20 enforcement tests) and _cim_trace.test.ts (the live decision trace on the CIM block). See contract.ts for why it is not yet a runtime caller.',
