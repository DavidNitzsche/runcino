...
export const MODULE_ORPHANS: Record<string, string> = {
  ...
  // ADJUDICATION-WIRE-1 (2026-09-04) · `adjudication/contract.ts` and
  // `adjudication/adjudicate.ts` LEFT THIS LIST. Their entries said the layer
  // was "test-only TODAY and deliberately so", named the wiring as the open
  // deliverable, and gave the honest reason it had not happened. It has now:
  // `lib/plan/validate.ts` §12 calls the layer through
  // `adjudication/from-plan.ts` on every path that authors a plan, and
  // `lib/plan/adaptive-ramp.ts` calls it through `adjudication/promotion.ts`
  // at the point a promoted volume bump would land. The entries are stale
  // rather than deliberate, which is exactly what the staleness ratchet in
  // GUARD 5 is for, and it is what caught them.
  //
  // The old entry's stated blocker was real and is resolved rather than
  // ignored: `checkPromotion` returns mayPromote=false on a block it cannot
  // size, so wiring it naively WOULD have blocked authoring for a live runner.
  // What binds now is the layer's REAL findings, always; the absent-history
  // refusal is recorded and excused only for the two callers that genuinely
  // cannot read a history, by name and with an argument, in
