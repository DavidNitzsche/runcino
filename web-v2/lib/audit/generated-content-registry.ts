...
export const MODULE_ORPHANS: Record<string, string> = {
  ...
  'web-v2/lib/fitness/fitness-model.ts':
    'THE LARGEST UNREAD ANSWER IN THE APP, and it is the one rule one exists for. Model A of the adaptive-progression split: "what can this athlete race today?", published as a RANGE with a confidence tier — "HM fitness estimate: 1:38-1:40, confidence: high" — precisely so a modelled number cannot read as a measured one. Its only importer is /api/coach/read, which nothing calls. Dies or lives with that route.',
  'web-v2/lib/coach/recommendation.ts':
    'Model C. action · change · reason · consequence · confidence. Its own header states the gap it closes: "The fitness model returns a range and a confidence tier; the adaptation model returns high/medium/low. Both get discarded at the copy layer, so a guess and a certainty read identically to the runner." That is still true, because its only importer is /api/coach/read, which nothing calls.',
...
export const ROUTE_CALLERS: Record<string, string> = {
  ...
  'web-v2/app/api/coach/read/route.ts':
    'THE BIGGEST ONE. "The whole coaching read in one call: what the runner can race today, how well they are absorbing training, what is actually holding the goal back, and what to do about it." Its own header names the surfaces that should consume it — web Today, the block view, iPhone Today, the run recap, the coach log — and none of them do. It also says, of the state BEFORE it was written: "the models were pure functions with no consumers · correct, tested, and invisible to the person doing the running." Building the route did not change that; it moved the invisibility one layer out. Wiring one surface to it retires four entries in this file at once (fitness-model, recommendation, and this route).',
