/**
 * _probe_cim_real.test.ts · TEMPORARY AUDIT HARNESS, sibling of
 * _probe_cim_block.test.ts, differing in ONE line that turned out to matter.
 *
 * The original probe authors at 2026-08-31T19:00:00Z — NOON PT on the 31st.
 * The plan-drift cron that actually authors this block fires at 04:00 UTC on
 * 08-31, which is 21:00 PT on 08-30, and the engine then resolves
 * `todayISO: 2026-08-30`. Those are different days to `runnerToday`, and they
 * produce materially different blocks: at noon-on-the-31st the first authored
 * week came out at 37.5 mi with 4.0 mi easy days; at the real tick it is 43.5
 * with 3.5/3.5/4.0 and a 9 mi medium-long. Verifying the wrong instant is how
 * a plan gets signed off on numbers that never ship.
 *
 * Same seam as its sibling (fake only `Date`, so pg's socket timeouts survive),
 * same OFF-by-default guard, same "read it, do not assert on it" posture.
 * Writes /tmp/cim-block-real.txt.
 *
 *   FAFF_CIM_PROBE=1 npx vitest run lib/plan/_probe_cim_real.test.ts
 */
...
const AUTHOR_INSTANT = new Date('2026-08-31T04:00:00Z'); // 21:00 PT on 2026-08-30 — the ACTUAL cron tick
...
describe.skipIf(!RUN)('CIM block as the cron will author it', () => {
  it('composes and prints', async () => {
    ...
    if (!r.ok) {
      require('fs').writeFileSync('/tmp/cim-block-real.txt', 'COMPOSE FAILED: ' + r.reason);
    ...
    out.push('\n=== PACE ANCHORS ===');
    {
      ...
      out.push('authoredState.goal_realism = ' + JSON.stringify(as.goal_realism));
    }
    ...
    require('fs').writeFileSync('/tmp/cim-block-real.txt', out.join('\n'));
