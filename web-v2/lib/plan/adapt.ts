...
export type AdaptationTriggerKind =
  ...
  | 'heat_bail'           // DEPRECATED 2026-08-27 · no longer emitted (the
                          // runner paces off feel, not a heat model). Kept so
                          // any in-flight coach_intents rows from the old
                          // path still resolve · see the 'heat_bail' case.
...
export interface AdaptationAction {
   ...
   * NARROWED 2026-08-29 · DIRECTION-1 · see below.
   *
   ...
   *
   * It was introduced so a convergent-red readiness morning could skip the
   * banner, on the 2026-08-19 ruling that such a change "is settled the night
   * before". THE OWNER REVERSED THAT on 2026-08-29: asked plainly whether he
   * wanted an automatic pull-back, he said no, and gave the general rule —
   * load may rise unattended, it may never fall unattended.
   *
   * INERT FOR ROUTING as of DIRECTION-1. `partitionActionsForCron` no longer
   * reads it at all. What it used to express is now derived from the action
   * itself, which is strictly safer:
   *   · a record-only `note` applies because it is a note (nothing to
   *     approve, and proposing one would delete it — `writeWorkoutProposals`
   *     drops actions with no `workoutIds`);
   *   · a `downgrade` or `shave` proposes because it takes work away.
   * Neither answer can be overridden by a flag, which is the point: the flag
   * existed to bypass the gate, and the gate is now the owner's rule.
   *
   * Kept on the type rather than deleted because several limbs still set it
   * and it is accurate DOCUMENTATION of intent — "this action is meant to
   * land unattended" — for the notes that do. It grants nothing. If you are
   * reaching for it to make a plan change apply without asking, the answer is
   * no; that is the rule, not an oversight.
   */
 ...
 *
 * ── DIRECTION-1 (2026-08-29) · THE OWNER'S RULE, IN ONE SENTENCE ───────────
 *
 * "Increasing is okay. Decreasing, or changing things from tempo or work to
 * easy, is not."
 *
 * This supersedes the 2026-08-19 ruling that a convergent-red readiness
 * morning is "settled the night before". Asked directly whether he wanted an
 * automatic pull-back, the owner said no — and then generalised it himself:
 * load may rise without approval, it may never FALL without approval. So the
 * axis that decides this set is no longer "is this an engine opinion or a
 * reaction to something that already happened"; it is WHICH DIRECTION THE
 * ACTION MOVES LOAD.
 *
 ...
 *     days stacked — an increase, but not one anybody chose. See the
 *     `onlyIfRescheduledId` coupling in the missed_key_workout limb.
 *
 * `progression_gate` stays OUT (2026-08-17, and still correct under the new
 * rule): it RAISES the dose on a session the runner has not run yet, so the
 * direction test admits it. Same for `pr_bank`, `goal_changed` and the
 * adaptive ramp — none of them takes work away.
 *
 * The direction rule is enforced structurally, not just by this list: see
 * `AdaptationAction.forceApplyNow`, which can no longer carry a plan-mutating
...
/**
 * Action kinds that TAKE WORK AWAY — the ones DIRECTION-1 says may never
 * apply without the runner's say-so.
 *
 * `note` is absent and that is the point: a note writes a `coach_intents` row
 * and changes no plan row, so it is not a load change at all. Keeping notes
 * applyable is what lets the engine record "we judged this morning red"
 * without touching the session — which the adaptive ramp's 48h guard then
 * reads so it cannot push load UP on the back of a red night nobody acted on.
 */
export const LOAD_REDUCING_ACTION_KINDS: ReadonlySet<AdaptationAction['kind']> =
  new Set<AdaptationAction['kind']>(['downgrade', 'shave']);

/** Does this action take work away from the runner's plan? */
export function reducesLoad(a: AdaptationAction): boolean {
  return LOAD_REDUCING_ACTION_KINDS.has(a.kind);
}
...
export function partitionActionsForCron(actions: AdaptationAction[]): {
  ...
  for (const a of actions) {
    // DIRECTION-1 (2026-08-29) · three rules, in order. The axis is what the
    // action DOES, not which trigger emitted it.
    //
    // 1. A record-only action never proposes. A `note` writes a
    //    `coach_intents` row and no plan row, so there is nothing for the
    //    runner to approve — and proposing one does not merely ask a pointless
    //    question, it DESTROYS the record: `writeWorkoutProposals` skips any
    //    action with no `workoutIds`, which every note has none of. That is
    //    how the readiness audit trail went missing, and with it the row
    //    `tryAdaptiveBump` reads before raising load. Structural, not
    //    flag-dependent: the old code only applied notes that happened to
    //    carry `forceApplyNow`, so a note without it silently evaporated.
    //
    // 2. A load-reducing action always proposes. The owner's rule is that load
    //    may rise unattended and may never fall unattended, so a `downgrade`
    //    or `shave` is gated REGARDLESS of `forceApplyNow`. Written as an
    //    explicit term rather than by trusting callers not to set the flag:
    //    bypassing this gate is the very thing the flag was invented for.
    //
    // 3. Otherwise the trigger kind decides, as before. This is what keeps
    //    `missed_key_workout`'s reschedule with its offsetting downgrade —
    //    the reschedule reduces nothing, but its trigger is gated, so the pair
    //    travels together and cannot leave two quality days stacked.
    const recordOnly = a.kind === 'note';
    const proposes = !recordOnly
      && (reducesLoad(a)
        || (a.sourceTrigger != null && PROPOSE_FIRST_TRIGGERS.has(a.sourceTrigger)));
