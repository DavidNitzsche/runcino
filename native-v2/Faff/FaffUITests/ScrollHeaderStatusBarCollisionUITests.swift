...
final class ScrollHeaderStatusBarCollisionUITests: XCTestCase {
    ...
    // MARK: - 4 · Stale banner composition — DELETED (F024, 2026-09-14)
    //
    // `testStaleBannerNeverLeavesABlackGapBehindTheStatusBarClock` proved
    // `.v5StaleBanner`/`.v5ScrollSafeTop` composed in the right order. David
    // rejected the banner itself outright (OD-20260914-003: "If I see the
    // banner again I'm going to lose it") — `StaleBannerV5`,
    // `V5StaleBannerModifier` and every primary-surface call site are gone
    // (`StaleStateV5.swift`, `HostsV5.swift`), so there is no banner left for
    // this test to assert a Retry button or a composition order against. A
    // banner-only test kept alive after its subject is deleted is exactly the
    // "dormant second truth" the owner-direction doc forbids.
}
