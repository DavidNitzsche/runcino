...
## The coach explains its own reasoning like a coach, not a readout (locked 2026-09-15)

David, relayed via David's Desk, 2026-09-15, on the new "why" feature (independent review's
PO-20260914-003): *"The 'why' is good because the more the app can feel like a conversation. Like
a coach saying 'here's what I think and how I can help and what lead me to this' etc the better."*

This is standing voice philosophy, not a note scoped to one component. **Anywhere the app explains
its own reasoning to the runner should read as a coach talking through his own thinking, not a
data or confidence-score readout.** That is the Layer 2 — Explanation surface named in
`docs/PRODUCT_UX_SIMPLIFICATION_DOCTRINE.md` ("available on 'why?'") wherever it actually lives:
the shared "why?" disclosure pattern proposed in PO-20260914-003, its existing one-off precedents
(`WorkoutResultV5`, `TodayBeforeV5`, `ProposalDetailV5`, `RunDetailV5`, `lib/faff/why-voice.ts`,
`lib/faff/explanation.ts`), post-run recap explanations (`lib/coach/run-recap.ts`), and any future
surface where the coach is effectively saying "here's what I think and why."

This is "The pattern" above (observation → reality check → reason → action), applied specifically
to the moment the coach shows its own work. Name what it's looking at, say what that means, say why
it leads to this conclusion — in the coach's own voice. Not `confidence: 0.72`, not a bare metric
dump, not "source_mode: RACE_DERIVED." The engine-layer provenance data (Layer 3 — evidence,
anchors, `source_mode`, uncertainty bands) is real and should inform what gets said, per the same
UX doctrine's own rule that Layer 3 "must never leak directly into Layer 1" — sophistication is
felt through what the coach says and how confidently he says it, not through more UI or exposed
internals. *"Based on two races, both in the last six weeks"* is the coach talking. *"Confidence:
HIGH, n=2, window=42d"* is the engine talking through the coach, and this line is what that's not
allowed to happen.

**Where this matters most, David's own extension (2026-09-15):** *"Pre run and post run would be
such strong moments for the conversation coach."* Pre-run and post-run are the two highest-leverage
moments for this register specifically — not only the "why?" disclosure component. Pre-run: the
run-lobby "here's the plan" moment, wherever a session is presented before it starts. Post-run: the
recap (`lib/coach/run-recap.ts`) and anywhere else a completed session gets talked through. These
two moments are where a runner is most likely to actually be listening for the coach's own
reasoning, not just a result — weight review and copy effort accordingly when scoping PO-008 or
any other pre/post-run surface, ahead of lower-leverage moments this same register also governs.
