...
This project is a running app for a competitive marathoner. **Locked 2026-08-31: iPhone is the focus, Apple Watch is its execution-layer partner. The web frontend is IGNORED UNTIL FURTHER NOTICE** — David's own words, not a permanent removal: don't propose it, don't fix it, don't spend effort on UX/design parity there, until he says otherwise. The backend (`web-v2/lib`, `web-v2/app/api` — the engine, the database, the API surface) stays and remains canonical, since iPhone and watch both consume it; what's paused is the web *frontend* (`web-v2/app/**` pages, `web-v2/components/faff-app/**`) as a product surface. Where a required-reading doc below still describes web as a co-equal surface ("command center"), read it as historical/backend-shared context only, not as active product scope.
...
- **The app explains itself like a coach talking through their own reasoning, not a data readout.** Locked 2026-09-15, David direct, verbatim: *"The 'why' is good because the more the app can feel like a conversation. Like a coach saying 'here's what I think and how I can help and what lead me to this' etc the better."* Said in response to a proposal for a reusable "why do you believe this" component (`PO-20260914-003`), but the principle is standing, not scoped to that one feature: any place the app explains a number, a decision, or a change to the runner — not just a dedicated "why?" affordance — should read as a coach walking through their own thinking in conversation ("here's what I think, here's what led me to it, here's how it helps"), never a confidence decimal, an evidence count, or a bare data-transparency panel. This extends the existing coach-voice rule above; it does not relax it — still short, still direct, still no hype.
