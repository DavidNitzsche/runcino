...
- **Peek** (always visible): a small white dot + run title + subtitle on the left; pace (Oswald) + effort label on the right. The **whole peek (grab + peek) is filled with the run's effort color** (`var(--accent)`) with white text, so the header carries the run-type color system. Rounded top corners; flows into the cream body below.
...
## Pre-run vs Post-run · the sheet has two states

The slide-up sheet renders one of two bodies depending on whether the day's run is **planned** or **done**. The prototype exposes a **Pre-run / Post-run picker below the phone (review chrome only — does not ship)**; in the app the state is driven by `d.done`.

The **effort color system** colors the peek in both states: pre-run = the run's effort accent (orange tempo, green easy, amber long, red intervals, cyan recovery, grey rest); **post-run = a solid emerald `#1f9a6f`** to read as "completed". Peek text is white in both; the completed peek swaps the dot for a **white circle + green check** and the effort label for a white **DONE** pill. Use one solid fill across grab+peek (a per-element gradient produces a visible seam at their junction).

### Pre-run body (PlannedHeroV2) — already covered above
Header (`{dw} · {TYPE} · PLANNED/TODAY` + Oswald title) · optional **adaptation banner** ("Adjusted from {original} · Restore") · stats trio (Distance / Target pace / Est time) · **effort target** Z1–Z5 gradient bar with the marker bubble · Conditions (Forecast / Best window / Shoe picker / Fuel) · Session segments + **CUE** line · **The Plan** (Upcoming/Today tag, verdict, recap, Heart rate / Effort / Cadence targets) · **Skip this run**.

### Post-run body (CompletedHeroV2)
When `d.done`, the sheet shows results instead:
- **Header**: `{dw} · {TYPE} · DONE` eyebrow + Oswald title + a green **win line** (check + `result.win`, e.g. "Held the line · 6:38 dead even").
- **Stats trio**: Distance / **Avg pace** / **Moving** time (real values).
- **Secondary stats** row: Avg HR / Elev gain / Conditions (actual `start → end` temps).
- **Route map**: dark card with the polyline in the run accent, start/finish dots, and a `X.X MI · ↗ Y FT` overlay. (In-app: tap to expand full-screen; the brief suggests a modal over inline pan-zoom.)
- **Mile splits**: one row per mile · number + a pace bar (width ∝ relative pace) + pace + HR, **color-coded by phase** (work = run accent, warmup/cooldown = teal `#5bbfb0`).
- **Form** grid: Cadence / Ground contact / Vert osc / Power (only the metrics present; grid columns = count).
- **How it went**: `On plan` tag + verdict + recap + a **planned-vs-actual targets** list (`actual vs target` per Heart rate / Pace / Cadence).
- **Start bar** becomes **Share run** (hidden on rest).
- The web's RPE 1–10 entry + post-run check-in chips are **intentionally omitted** here (cut by design); re-add later if the app wants self-report capture.
...
- `RUNS` — per run-type **planned** data: `accent, name, tag, title, effort, pin (effort-bar position), zone, dist, pace, est, wx (forecast), window, shoe, fuel, segs[], cue, planH, planD, targets{hr,eff,cad}, adjusted?`.
- `RESULTS` — per run-type **completed** data: `win, time, paceA, hr, elev, cond, cad, gct, vo, pwr, verdict, recap, cmp[][planned,actual]`. Mile splits are generated from `paceA` + mile count + a per-type phase pattern.
