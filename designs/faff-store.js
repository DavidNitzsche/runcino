/**
 * faff-store.js — central data hub for the v4 local build.
 *
 ...
 *
 * Usage in a page:
 *   <script src="faff-store.js"></script>
 *   const s = FaffStore.getState();
 *   FaffStore.logCheckIn({ energy: 7, soreness: 3, stress: 2 });
 *   FaffStore.subscribe((next, prev) => renderFromState(next));
 */
(function () {
  'use strict';

  const STORE_KEY = 'faff_v4_state';
  ...
  console.log('[faff-store] loaded · today =', getState().today, '· check-ins logged:', Object.keys(getState().checkins).length);
})();
