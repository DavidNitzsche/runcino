/**
 * faff-bind.js — auto-render bindings from FaffStore on load.
 *
 ...
 *
 * Add the script with: <script src="faff-bind.js" defer></script>
 */
(function () {
  'use strict';
  ...
  function format(state) {
    ...
    const todayLabel = FaffStore.fmtDayLabel(today).toUpperCase();
    const ctx = FaffStore.getCurrentWeekContext();
  ...
  function render() {
    if (!window.FaffStore) return;
    const state = FaffStore.getState();
  ...
  if (window.FaffStore && typeof FaffStore.subscribe === 'function') {
    FaffStore.subscribe(render);
  }
})();
