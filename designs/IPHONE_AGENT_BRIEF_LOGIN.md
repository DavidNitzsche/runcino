...
| Your merge lands | `SHIPPED · commit <sha> · IOS` (or `· BOTH` if web also shipped) |

Grep `docs/BACKEND_FRONTEND_COVERAGE.html` for `DESIGN-READY` rows whose
Found-by matches your platform (or `BOTH`) · that's your queue. Pull
highest-leverage rows first per the design memo's ranking.

If a row already shows `SHIPPED · WEB`, check whether it also makes sense
on iOS · if yes, implement and update the prefix to `SHIPPED · commit <sha> · BOTH`.
