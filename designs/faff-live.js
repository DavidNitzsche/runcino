/**
 * faff-live.js — best-effort live data refresh for the local v4 build.
 *
...
(function () {
  ...
  fetchJSON('/api/overview')
    .then((d) => {
      ...
      window.FAFF.RUN_LIVE = { ok: true, fetchedAt: new Date().toISOString(), data: d };
      console.log('[faff] live data loaded', { today, nextA });
    })
    .catch((err) => {
      window.FAFF.RUN_LIVE = { ok: false, reason: err.message };
      console.log('[faff] live data unavailable, using embedded snapshot:', err.message);
    });
})();
