...
| `resting_hr_override` | int | 25–120 · API-only — keep RHR **display-only** (show measured `rhr`), matching web. Don't build an editor for it. |
...
### Seeding the editor — there are TWO reads, not one
- **`GET /api/profile`** now also returns `email`, `weekly_frequency`, `weight_kg`, `timezone`, `tz_mode`, `max_hr_override`, `resting_hr_override`, `fuel_brand`, `fuel_gel_carbs_g`, `fuel_target_g_per_hr` (plus all existing fields). Seeds **YOU / PHYSIOLOGY / RACE FUELING / TIMEZONE**.
- **`GET /api/settings`** returns the schedule + notif + units block: `long_run_day`, `rest_day`, `quality_days`, `briefing_time`, `push_enabled`, `units_*`. Seeds the **TRAINING day-of-week controls**. The schedule fields are NOT on the profile response — don't look for them there. **The native layer already wraps this:** `API.fetchSettings()` (API.swift:324) returns `UserSettings` (:1104) which already decodes all four schedule fields. Read from it to seed those rows. Do NOT keep the hardcoded placeholders (`ProfileView.swift:306–308` → `"07:00"/"Saturday"/"Monday"`) — re-hardcoding is the exact bug this brief is killing.
...
`ProfileFields` (:1113) — add the new optional fields above. **It has neither `email` nor `full_name` today — add both** (identity fields, easy to miss since the rest are numeric/fuel). `updateProfile(patch)` (:160) and `patchSettings(patch)` (:331) already send arbitrary keys, and `fetchSettings()` (:324) already reads the settings block — so no new methods needed. `setRunnerTimezone` lives server-side; just PATCH `{timezone}` or `{tz_mode}`.
...
## Reuse
ProfileView `weeklyMiSheet` stepper (:294–385) is the edit-sheet template for numbers. SettingsView `segment()` for 2-way pickers — but experience (4 options) and sex (3 options) need a multi-option picker, not the 2-way `segment()`. Chips for multiselect (quality days, cross-training). For the manual timezone picker, mirror web's curated `ZONES` list (`SettingsPanel.tsx:82`) — don't enumerate the full IANA database. Web reference implementation (field list + generic editor): `web-v2/components/faff-app/views/SettingsPanel.tsx`.
