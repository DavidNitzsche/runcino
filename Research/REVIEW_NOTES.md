...
- **01 pass executed (owner-approved 2026-08-28):** both residuals corrected in 01 itself. The 15K column was regenerated for every row by bisection solve of the doc's own §"How VDOT is calculated" equations (VO2(v)/%VO2max(t) = VDOT at D = 15,000 m); the printed values had drifted up to ~1.7 VDOT slow by row 85 (implied VDOT 83.3). The Mile column was confirmed to hold 1500m solutions (implied VDOT within ±0.2 of the row label at 1500 m through VDOT ~58, drifting toward published-1500 values above) and was **recomputed as true mile (1609.34 m) solutions, keeping the "Mile" label** — relabelling to "1500m" was rejected because the doctrine gate's `PACE.repetition-is-mile-race-pace` claim walks the table by the `Mile` header and binds the engine's R pace (defined as mile race pace) to it; the recompute keeps every anchor resolving and makes the column mean what its label and its consumers say it means. The recomputed mile solves also land on Daniels' published mile times (VDOT 30 → 9:10 vs published 9:11). 5K/10K/Half/Marathon columns re-verified returning their labelled VDOT ±0.1 and untouched; 3K left as the transcribed published column. The table footnote now states which columns are equation-solved. Downstream, executed in the same pass: 02 §5.1's 15K cells re-copied from corrected 01 (row 35 again the mean of 01's 34/36 rows) and its 10mi column re-interpolated in log-time from each row's own corrected 15K and unchanged Half anchors; `web-v2/lib/training/vdot.ts`'s `MILE_VDOT_TABLE` transcription updated to the corrected column (its old values priced R ~25–40 s/mi too fast and under-read mile-goal VDOT by ~4), and the mile-correction window narrowed to 0.98–1.05 mi so a 1500m finish is no longer read as a mile finish.
