...
- **startLocal canonicalization backfill — gated write session.** Canonicalize all `startLocal` → true-UTC via `toUtcIso()`. This is the root-cause fix for the `isSameRun` false-negative class. After backfill, `Z`=UTC is uniformly trustworthy and the strava strip can eventually go too. 8 legacy rows currently flag-protected, no prod impact, no urgency before AFC.
  - **Context (2026-06-09 dedup health check):** the false negatives are legacy *multi-ingest* — the same physical run was stored 3–4× with **mixed `Z` conventions** (e.g. 05-22 `apple_health` exists as BOTH `…10:00:31Z` spurious-local AND `…17:00:31Z` true-UTC), which **no source-based strip can reconcile** — `apple_health` can't mean both "strip" and "trust". A read-time `apple_health`/null strip was tried (`846f3509`) and **reverted** (it clustered the spurious-Z dupes but broke the legit-UTC ones; net 8→6 stale, goal not met). Back to strava-only. `volume.ts` is correct throughout (pre-filters flags → 27 runs / 176.69 mi). Diagnostic kept at `web-v2/lib/runs/_dedup-health.audit.test.ts` (RO-gated).
...
## Audit E — Fixes E3 / E4 / E5 / E6  [DEPLOYED 2026-06-08 · origin/main · tsc 0 · vitest 380/0/3]

All four MAJOR Audit-E fixes shipped (David approved each; deployed via the normal pipeline). RO falsifiers run against prod data; authed recap/poster endpoints can't be exercised from the RO session, so the deployed logic is proven by tsc + tests + prod-RO reproductions (deterministic from the same data).

- **E4 — pace from string** (commit `bd7b85d4`) · `recap/route.ts`. `actualPaceSPerMi` was null on every watch/HK run (read only `paceSPerMi`); now `Number(paceSPerMi) || parsePaceToSec(avgPaceMinPerMi)`, single source for deriveRecap + deriveWin. Numeric wins when present (Strava); cold-start safe. Falsifier: 4 real runs null → 489/461/501/481s.
- **E6 — type-aware conditions copy** (commit `e4d8d088`) · `weather-adjust.ts` + `run-recap.ts`. Optional `workoutType` on `WeatherInput`; slowdown NUMBER unchanged (goal-projection unaffected). easy/long/recovery/shakeout reframe summary + tip around effort/HR; tempo/threshold/intervals/race + unknown keep pace framing (back-compat). Falsifier: real Jun 5 easy (9.7% hot) → "Hold the effort / go by effort" vs tempo → "Costs you ~10% on pace".
- **E3 — frozen target is the evaluation contract** (commit `311ddc98`) · `recap/route.ts`. recap/win now judge vs the FROZEN prescribed work-phase target (mode across reps), falling back to live `plan_workouts` only when no frozen phase exists (non-watch/manual/cold-start) — matching the phase panel (already frozen). A re-pace can no longer retroactively flip a missed rep to a hit. Light "Plan now reads X/mi · re-paced after you ran" note appended to `facts` (all 3 recap surfaces, no renderer change) when live diverges ≥10 s/mi. Response exposes `prescribed/plan_now/evaluated_pace_s_per_mi`. Falsifier: Jun 2 eval 6:29 + note 6:52; Jun 4 false `winTempo` "held the line" no longer fires.
- **E5 — honest done-state (web-only)** (commit `83f4a9ae`) · `glance-state.ts` + `glance-adapter.ts` + `glance-adapter.test.ts`. New `todayExecution` verdict derived from the frozen watch-completion phases (the only signal that catches a missed-rep run whose mileage matched the plan — doneMi vs plannedMi can't). resolveDayState: overreach (doneMi ≥ 1.25× planned) → `done_ease_off` (its real purpose, finally wired); a SHORT session keeps the `done_nailed` STATE but flips verb/stat-tile/prose → "CAME UP SHORT" / "◑ PARTIAL" (amber) / "Came up short. That's data · note what was hard and bring it to the next one." **No new DayState → no iPhone FaffDayState enum change.** **Only WORK phases count — cutting a warmup/cooldown short is NOT "short" (David's call); 'short' fires on a work phase not completing or ≥⅓ of ran work phases missing pace.** Falsifier: Jun 2 (2/4 missed) + Jun 4 (tempo 18 s/mi slow; cooldown was the abandoned phase) → short; Jun 5/7 → nailed; synthetic tempo-hit + cooldown-cut → nailed.
- **Audit's E5 one-liner was wrong (falsified):** (a) the surface is the WEB /today poster — the iPhone briefing path (`reciteToday`) renders the planned workout with no done-verdict; `PosterPayload`/`FaffDayState` are defined but never constructed; (b) "majority missed → done_ease_off" is backwards — `done_ease_off` = overreach, not fell-short; (c) the verdict isn't in GlanceState — it had to be plumbed from the frozen phases.
- **E7–E10 (MINOR)** — still queued (below).

### E5 follow-up — heat-adjust the done-state verdict  [DEPLOYED 2026-06-08 · commit `e0381c0f` · prod confirm ✅]
First-shipped E5 trusted the watch's on-device (weather-unaware) `verdict`, so a run executed correctly for the heat read "CAME UP SHORT" on the done-state while the phase panel (`loadPhaseBreakdown`) — which already heat-adjusts — showed it "on". Same screen, opposite verdict. `computeTodayExecution` now derives `slowdownPct` via `judgeWeather` (same call `computeHeatSlowdownForRun` uses) and judges each work phase against the heat-honest target + 10 s/mi tolerance — identical band math to `loadPhaseBreakdown`; the watch `verdict` field is no longer consulted. "short" now means missed even after the heat allowance, or the quality block abandoned mid-work. **Prod confirm (RO, post-deploy):** Jun 2 (11.4% slowdown) + Jun 4 (7.9%) flipped short → **nailed**; Jun 5/7 unchanged. tsc 0 · vitest 380/0/3. Full evidence in `OVERNIGHT-REPORT.md`. Note: `slowdownPct` is NOT stored on the row — derived at read time.

### OPEN follow-up — recap win line is weather-unaware (winTempo / winLong)
`lib/coach/run-win.ts` `winTempo` / `winLong` compare actual vs target with a **raw** delta — no heat adjustment — so they're inconsistent with `loadPhaseBreakdown` and the now-heat-aware E5 done-state. **Not mis-firing on the current runs** (winTempo receives the whole-run avg pace, which is far from the segment target, so it returns null for Jun 4), so this is latent, not live-wrong. But on a run where the whole-run avg lands near a target in heat, the win line could fire a heat-unaware "held the line" that contradicts the heat-adjusted verdict. **Fix:** thread the run's `slowdownPct` into `deriveWin` and heat-adjust the `winTempo`/`winLong` comparison with the same band as `loadPhaseBreakdown` / `computeTodayExecution`. Consider extracting a shared `heatAdjustedStatus(target, actual, slowdownPct)` helper so the phase panel, the done-state verdict, and the win line all use one implementation (currently three copies of the band math). **Needs its own fix session.**

## Audit E — Fix Queue (logged 2026-06-08)

### MAJOR — DONE 2026-06-08 (E3–E6 deployed; see section above for what shipped)
...
**Shipped (3 pieces · David's GO · on-read baseline decision: app-tracked miles only, fictional seeds discarded — 0 is honest):**
- **Auto-assign at ingest** — new `lib/shoe/auto-assign.ts` (`assignShoeIfMissing` + `planTypeToShoeType` mapper) wired into the single post-write chokepoint `lib/runs/post-write-hooks.ts afterRunWrite` (covers watch + HK + manual). Priority: day_actions pick → Strava gear → `recommendShoe`. Sets `shoe_id` + `shoe_auto_assigned_at` only when null (guarded/idempotent). Gear matcher extracted to `lib/shoe/gear-match.ts` (one source; pullSync repointed — no 3rd copy).
- **/today reconciliation** — `POST /api/today/shoe` now also updates `runs.shoe_id` for any same-day run (fills empties + overrides prior auto/day-pick assigns; preserves per-run modal picks, which carry a NULL stamp). The dead write is now the highest-priority auto-assign input.
...
- **16-D · Shoe settings / management UI** — own session, UI build. Runners need to set per-shoe metadata IN THE APP, not via DB write (today `run_types` is only settable by a direct `UPDATE shoes`). The auto-assign + picker recommendation are only as good as the `run_types` tags, so this is the upstream control surface for the whole feature. The shoe management modal needs: (1) a **`run_types` selector per shoe** (which session types this shoe is for — easy/long/tempo/intervals/recovery/race/as_needed) driving `recommendShoe`; (2) **mileage display** (on-read, already correct via `lib/shoe/mileage.ts`); (3) a **primary/preferred toggle** to disambiguate when multiple shoes cover the same type (note: today all of David's 7 shoes are `preferred=true`, which makes the preferred-tiebreak a no-op — the toggle should be mutually-meaningful per type). Bundle the 16-A starting-mileage field into the same modal. PATCH path already exists (`/api/shoe` accepts `run_types`/`preferred`/`mileage_cap`); this is the missing front-end.
