...
// =====================================================================
// RPE PROMPT (Tier 2 · post-rep subjective effort capture)
// =====================================================================

/// Rate-of-Perceived-Exertion prompt overlay. Shown by the engine during
/// the recovery phase that follows a completed `.work` rep. 30-second
/// auto-dismiss; tap 1-5 to rate, swipe down to dismiss without rating.
///
/// Layout sits on the locked law's top-label baseline (rides the OS
/// clock) but uses interactive buttons instead of big-number rows
/// because the body is action, not data. Five 80pt circles centered
/// vertically — fat tap targets for jogging-and-tapping.
///
/// Color encodes severity: green (1, easy) → blue/white (3, prescribed)
/// → amber/red (5, max). The runner's eye lands on the color matching
/// their feeling before they read the number, so they can rate without
/// composing.
struct RpeFace: View {
    /// Called when the runner picks a rating. `rating` is 1-5.
    var onPick: (Int) -> Void = { _ in }
    /// Called on down-swipe dismiss without rating.
    var onDismiss: () -> Void = {}
    /// Remaining auto-dismiss seconds (0-30); rendered as a small caption.
    /// Engine ticks this down; `nil` hides the countdown row.
    var secondsRemaining: Int? = nil

    /// Color per rating · matches Faff palette so it feels native to
    /// the rest of the locked grammar.
    private func color(for n: Int) -> Color {
        switch n {
        case 1: return Faff.live    // easy · green
        case 2: return Faff.rest    // light · cool blue
        case 3: return Faff.ink     // moderate · neutral white
        case 4: return Faff.goal    // hard · amber
        default: return Faff.over   // max · red
        }
    }
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            let buttonD = h * 0.155     // 80pt @ H=514 → fat tap target
            let spacing = (w - buttonD * 5) / 6
            ZStack(alignment: .topLeading) {
                Color.black.ignoresSafeArea()
                // Top label rides OS clock baseline (the locked law).
                // Single word so we never collide with the clock at top-right;
                // the prompt context (5 buttons during recovery) makes the
                // meaning unambiguous.
                Text("RATE")
                    .font(.custom("HelveticaNeue-Bold", size: h * 0.080))
                    .foregroundStyle(Faff.mute)
                    .padding(.top, h * 0.0739)         // = TOP_MARGIN
                    .padding(.leading, h * 0.060)      // = leadF

                // 5 rating buttons centered vertically. Each is a 80pt
                // circle with the digit + a meaning glyph stacked.
                HStack(spacing: spacing) {
                    ForEach(1...5, id: \.self) { n in
                        Button(action: { onPick(n) }) {
                            ZStack {
                                Circle()
                                    .fill(color(for: n).opacity(0.18))
                                    .overlay(Circle().stroke(color(for: n), lineWidth: 2))
                                Text("\(n)")
                                    .font(.custom("HelveticaNeue-Bold", size: buttonD * 0.55))
                ...
                // Auto-dismiss countdown caption at bottom. Honest read so
                // the runner knows the prompt won't linger.
                if let s = secondsRemaining {
                    Text("\(s)s")
                        .font(.custom("HelveticaNeue-Bold", size: h * 0.060))
                        .foregroundStyle(Faff.dim)
                        .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .bottom)
                        .padding(.bottom, h * 0.040)
                }
            }
            .gesture(
                DragGesture(minimumDistance: 24)
                    .onEnded { value in
                        if value.translation.height > 16 { onDismiss() }
                    }
            )
        }
    }
