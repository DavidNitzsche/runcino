...
// Apple value can sit in display surfaces but never bleed into
// paceTargetFromVdot / pacesFromVdot / vdotSnapshot.
//
// This test wires the actual pace pipeline (vdot.ts) and asserts:
//   1. paceTargetFromVdot output is identical with vs without an
//      Apple VO2max present.
//   2. Apple VO2max set at 65 (would cold-start to VDOT 55) does NOT
//      shift the prescribed pace away from the race-derived VDOT 47.
// ─────────────────────────────────────────────────────────────────────
...
describe('snapshot · race VDOT + Apple VO2max both present → paces use ONLY race VDOT', () => {
  ...
  it('prescribed paces ignore Apple VO2max when race data is present', () => {
    // Compute prescribed paces with NO Apple VO2max in the picture.
    const baselineEasy = paceTargetFromVdot(stateWithRace, 'general_aerobic');
    const baselineThreshold = paceTargetFromVdot(stateWithRace, 'threshold');
    const baselineVO2 = paceTargetFromVdot(stateWithRace, 'vo2');
    const baselineMarathon = paceTargetFromVdot(stateWithRace, 'marathon_specific');

    expect(baselineEasy).not.toBeNull();
    expect(baselineThreshold).not.toBeNull();
    expect(baselineVO2).not.toBeNull();
    expect(baselineMarathon).not.toBeNull();

    // Now introduce an Apple VO2max of 65 (which would cold-start to
    // VDOT 55, ~8 points higher than race-derived VDOT 47). If the
    // wellness signal had any influence on pace, the prescribed paces
    // would change. They MUST NOT.
    const apple = buildVo2MaxApple(65, '2026-05-15T10:00:00Z');
    ...
    const afterThreshold = paceTargetFromVdot(stateWithRace, 'threshold');
    const afterVO2 = paceTargetFromVdot(stateWithRace, 'vo2');
    const afterMarathon = paceTargetFromVdot(stateWithRace, 'marathon_specific');

    expect(afterEasy).toEqual(baselineEasy);
    expect(afterThreshold).toEqual(baselineThreshold);
    expect(afterVO2).toEqual(baselineVO2);
    expect(afterMarathon).toEqual(baselineMarathon);
  });
