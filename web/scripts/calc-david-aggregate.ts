...
for (const s of result?.sources ?? []) {
  ...
  console.log(`  ${s.canonicalLabel.padEnd(8)} ${s.date} (${days}d old) ${s.priority}: VDOT ${s.vdot.toFixed(1)}  weight=${s.weight.toFixed(3)}  recency=${s.weightBreakdown.recency.toFixed(3)} length=${s.weightBreakdown.length.toFixed(3)} tier=${s.weightBreakdown.tier} effort=${s.weightBreakdown.effort}  source=${s.source} goalTier=${s.isGoalTier} inCycle=${s.isInCycle}`);
}
const totalW = (result?.sources ?? []).reduce((sum, s) => sum + s.weight, 0);
console.log('');
console.log('Weight shares:');
for (const s of result?.sources ?? []) {
  console.log(`  ${s.canonicalLabel.padEnd(8)} ${s.date} (${s.priority}): ${(s.weight / totalW * 100).toFixed(1)}%`);
}
