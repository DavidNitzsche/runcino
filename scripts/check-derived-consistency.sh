...
lib/plan/plan-snapshot.ts :: PLANSNAPSHOT-1 (2026-09-03) · two halves, two standings. `treadmillGuidanceFor`'s `speedMph = 3600 / card.workPaceSPerMi` converts an AUTHORED prescription's pace into a treadmill speed — same standing as lib/watch/heat.ts's entry: a workout not yet run has no logged row and no contradictory figures, so there is nothing for coherentPace to reconcile it against, and routing it through the coherence readers would either refuse a valid prescription or invent a measurement. The matched/supplemental-run half is already routed: every distanceMi/durationSec/paceSPerMi triplet for a REAL run comes from one `runFacts(matchedRun.data, { basis: 'elapsed' })` call, destructured field for field, same shape as lib/postrun/detail-load.ts's entry below — it derives nothing independently, `runFacts` (lib/runs/run-facts.ts) is itself built on `reconcileRun` from lib/runs/coherence.ts. If this file ever starts dividing a distance by a duration for a MATCHED run outside `runFacts`, or reading a clock-family value off a real `runs` row directly, that is a real finding and this entry is where the argument lives.
...
lib/plan/plan-snapshot.ts :: PLANSNAPSHOT-1 (2026-09-04) · two unrelated things trigger this file's flags, neither a fresh reconciliation. (1) `matched_run`/`supplemental_runs` read `runFacts(r.data, {basis:'elapsed'})` for pace/duration — `runFacts` itself calls `reconcileRun` internally (its own header: "the judgement now happens once, in reconcileRun"), same standing as `lib/faff/v5-today.ts`'s entry, which this scanner also cannot auto-clear by import for the identical reason. (2) `treadmillGuidanceFor`'s `speedMph = 3600 / card.workPaceSPerMi` derives a treadmill speed hint from `workPaceSPerMi`, a PRESCRIBED pace target on a `SpecCard` the day may not have been run yet — there is no logged `runs` row behind it. Same standing as `lib/watch/heat.ts` and `lib/race/distance-doctrine.ts`: a prescription, not an observation, and there is nothing for the coherence readers to reconcile it against. If this file ever starts deriving pace/duration from `runs.data` directly (bypassing `runFacts`) or reading a MEASURED pace into the treadmill hint, that is a real finding and this entry is where the argument lives.
