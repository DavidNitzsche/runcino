...
lib/plan/plan-snapshot.ts :: PLANSNAPSHOT-1 (2026-09-03) · every distanceMi/durationSec/paceSPerMi it returns for a matched or supplemental run comes from `runFacts(data, { basis: 'elapsed' })` (lib/runs/run-facts.ts), which calls `reconcileRun` from lib/runs/coherence.ts internally — the scanner cannot see through that call boundary because the file imports run-facts, not coherence, directly. Same standing as lib/terrain/run-terrain.ts and lib/runs/log-enrich.ts above: handed already-resolved values, reads no stored key of its own. It never reads avgCadence, avgStrideLengthM or avgPaceMinPerMi at all — the cadence and display-string families it also trips share distanceMi/durationSec as members, not because this file touches cadence or a display string.
