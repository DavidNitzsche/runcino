...
lib/plan/plan-snapshot.ts :: PLANSNAPSHOT-1 (2026-09-03) · two distinct shapes, both already argued elsewhere in this list. (1) `treadmillGuidanceFor`'s speedMph is `3600 / card.workPaceSPerMi`, a PRESCRIPTION conversion off a plan-authored pace target — no `runs` row, no clock, no distance involved, same standing as lib/watch/heat.ts and lib/race/distance-doctrine.ts above ("a workout not yet run has no contradictory figures"). (2) `matched_run`/`supplemental_runs` derive durationSec and paceSPerMi from TWO calls to `runFacts(data, { basis: 'elapsed' })` (lib/runs/run-facts.ts, "THE COHERENT TRIPLE" resolver) against the SAME run's data with the SAME basis argument — already reconciled by that function, same standing as lib/execution/reconstruct.ts ("already reads through run-shape's accessors ... names family members while doing so"). If this file ever starts reading a stored pace/duration field directly off `runs.data` instead of through runFacts, or diffing two DIFFERENT runs' clocks, that is a real finding and this entry is where the argument lives.
