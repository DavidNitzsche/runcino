...
lib/runs/work-averages.ts :: SIMROW-1 · TODAY (2026-09-08) · THE OWNER OF THE WORK-SCOPED AVERAGE, and it names the family members only in `workStatsForDisplay`'s mapping out of `run-shape.ts#runPhases`. There is no run-level clock here to reconcile: every value is PER-PHASE and already resolved by that owner (`n.actualDurationSec`, `n.actualDistanceMi`, `n.avgHr` off `NormalizedPhase`, with cadence read off the same element by position because the normalised shape does not carry it). Nothing in this file reads `runs.data` at run level, compares a stored pace against a clock, or picks between moving and elapsed time -- the one pace it produces is `workSeconds / workMiles` summed across the work phases, a quantity no stored field states and therefore no stored field can contradict. Same standing as lib/execution/verdict.ts, which routes the identical phase mapping through the same `runPhases` owner into this module's own `workAveragesFromPhases`. THIS ENTRY IS A RELOCATION, NOT A NEW EXEMPTION: the mapping lived inline in app/api/v5/today/route.ts, which the scanner auto-cleared because that file happens to import lib/runs/coherence for an unrelated read -- a blanket clearance that never stated an argument. Moving the code here cost that clearance and bought this sentence, which is the trade the allowlist exists to make. If this file ever starts reading a RUN-level distance, duration or stored pace, that is a real finding and this entry is where the argument lives.
...
lib/evidence/classify-evidence.ts :: EVIDENCECLOCK-1 (2026-09-06) · same standing as lib/plan/plan-snapshot.ts's runFacts entry above, and for the identical reason: `runFacts(input.data, { basis: 'moving' })` is called ONCE at the top of `classifyContext`, and every clock member the scanner sees below it — `facts.elapsedSec`, `facts.timeSec`, the `movingSec` alias — is read off that ALREADY-RECONCILED return value, never off `input.data` directly. The scanner cannot see through `runFacts` to the `reconcileRun`/lib/runs/coherence.ts call it makes internally, which is exactly the gap plan-snapshot.ts's entry already names. Comparing `facts.elapsedSec` against `facts.timeSec` is deliberate, not an oversight: it is how pause detection asks whether the two clocks disagree, which is a different question from picking one to display, and doing it on the reconciled pair rather than two raw reads is the fix, not the violation. A second unreconciled `runMovingSec(input.data)` call previously existed here and was removed for exactly this file's own EVIDENCECLOCK-1 comment inline — if a raw call to `runMovingSec`, `runDistanceMi` or any other data-level clock accessor reappears in this file outside of `runFacts`'s own internals, that is a real finding and this entry is where the argument lives.
