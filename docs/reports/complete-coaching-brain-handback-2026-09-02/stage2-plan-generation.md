...
- ~~**The compound-progression finding is ADVISORY.** Binding it today refuses the owner's block for a cutback rebound doctrine licenses.~~ **CLOSED 2026-09-02 (STRESSOR-1)** — David ruled it binding: "Accidental combinations must fail plan generation rather than ship as warnings." The rebound was real and the reasoning was half right: the TEST was wrong, not just the enforcement. It compared long-run MILES, and a long run that grows with the week at a held share is one stressor expressed as two numbers (`Research/00a` §"Practical base-building rules" · "Long run grows | Up to 25–30% of weekly volume"). It now compares the long-run SHARE against doctrine's own five-point band, with a grid-coherence floor mirroring `SPIKE_MIN_COHERENT_ANCHOR_MI`, and the rebound is a TYPED, RECORDED exemption rather than a silent skip. Measured: 8,781 archetypes and the owner's live block, zero enforced findings; closest non-exempt week 3.85 points against the 5.00-point band. See `combined-stress.ts#compoundProgressionCheck`.
