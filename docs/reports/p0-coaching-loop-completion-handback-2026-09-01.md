...
| CIM training-to-race coherence | PASS | §8: bridge with value, evidence, confidence and change trigger at every step; race row 7:31/mi (3:17:00) = `outlook.execution`, MP rows 7:55/mi = `outlook.trainingPrescription`, tune-up row brought under the same refresh in `3c2d1eaa`+1 | The tune-up row was priced 7:05/mi (authoring seed) until this report's last commit; it is re-priced by the next canonical refresh and the ledger entry is in §11 |
...
| Full automated verification | PARTIAL | §13: tsc clean · prebuild 17/17 · `next build` exit 0 · Xcode `Faff` scheme BUILD SUCCEEDED (iphonesimulator + watchsimulator) · full Vitest 8119 passed / 10 skipped / 0 failed on `f967cab1` · CI `test-full` green on `331e6bab` and `3c2d1eaa` | The final commit of this report (tune-up refresh + watch decode) was pushed after this table was written; its CI run and Railway deployment are recorded in §2 as pending unless updated below it |
...
| 5 | `3c2d1eaa` | proof package, phone outlook section, staleness + race-row gates, CI acknowledgement of the credential-free audit skip (24 files) | test-full ✓ · build-check in progress at time of writing | in progress at time of writing |
...
| 2026-12-01 | 13 | race_week_tuneup · 5 mi | race-pace tune-up | — | — | — | — | — | 425 s/mi (7:05) until this report's last commit; **now priced by the refresh at the execution target (451)** | race_hr informational | was the authoring seed |
...
| `wko_60e4afb00a4e2c33` | 2026-12-01 | race_week_tuneup | no | 425 · — | unchanged by this run (not yet in scope); in scope from this report's last commit | — | — | pending the next refresh |

Future/unsealed rows changed: 4. Completed/sealed rows changed: **0** — `live-plan-ledger/p6-sealed-checksum-{before,after}.txt`: 7 sealed rows, md5 `1f9bc33de7f4cbb10c6807304305e1af` before and after. Stated goals unchanged: `p6-goals-{before,after}.txt` (CIM 3:00:00 / 10800, Malibu 1:30:00 / 5400, Dodgers 0:45:00 / 2700, Santa Monica none). Training rows: 0 changed by this run (`pace_recompute` stamp still 2026-08-31 `prescription_wire_1_promotion`, 77 rows; the reanchor found nothing to move).
...
- **Full-suite CI (`test-full`):** ✗ 63d47b5d (lint findings) · ✓ 331e6bab · ✗ 0bb6ac8c · ✗ f967cab1 (audit liveness without credentials) · ✓ 3c2d1eaa.
...
- **Full automated verification.** Blocker: the final commit's CI run and Railway deployment were pending when this report was written; §2 records them once complete. No code reason.
