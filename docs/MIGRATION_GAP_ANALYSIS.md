# faff.run Migration Gap Analysis
...
11. **`/training`, `/retrospective`, `/research`, `/settings/integrations` are noted as "broken" in STATUS.md** — they reference dead CSS classes (`.faff-card`, `.btn-accent`, `--color-paper`). When migrating to the May 9 designs, these existing files should likely be deleted rather than patched, since the new design system uses a completely different vocabulary.
