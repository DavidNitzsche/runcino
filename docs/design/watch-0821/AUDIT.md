...
## 12 · Commit one · the token layer (landed)

`WatchThemeV5.swift` + the two v5 typefaces + the palette gate, green in one commit.
No face touched, no legacy token deleted — the 0821 faces land one at a time behind a
self-expiring gate branch, so the wrist is never half-migrated on a device.

**The gate now has a watch section 1c/1d mirroring the phone's 1a/1b.** 1c locks the
0821 values. 1d asserts the legacy ten *while they still have consumers* and **inverts on
its own**: when the last face stops referencing `Faff.*` / `WatchTheme.C` / `Role.`, it
stops asserting the old values and starts demanding they be deleted. `WATCH_ALLOWED_HEX`
is likewise assembled from the legacy count, so the allowlist shrinks by itself.
Currently **9 face files** still on the legacy palette; the summary line prints the count
on every run.

Falsified rather than assumed: flipping `attention` to the old `#F3AD38` fails the gate
with the right message, and the legacy file list is the real nine.

### Two traps found while landing it

**1 · `BUILD SUCCEEDED` proves nothing about a new file.** `native-v2/Faff.xcodeproj` is
generated by xcodegen and **tracked in git**, so it goes stale the moment a file is added.
The first watch build passed with `WatchThemeV5.swift` absent from `project.pbxproj` —
not compiled, not linked, and no error, because the project simply did not know it
existed. Same for the two TTFs: the build was green and the fonts were not in the `.app`.

This is a sibling of the `swiftc -typecheck` trap, one layer up, and it is worse because
the signal is a *success*. **Run `xcodegen generate` after adding any file, then verify
the artefact** — `grep project.pbxproj` for the filename, and `find "$APP" -name '*.ttf'`
for resources. A clean regen touched exactly the three new files and nothing on the
iPhone target.

**2 · A self-expiring gate can hold itself open.** The first version of the 1d file scan
counted `WatchThemeV5.swift` as a legacy consumer, because its own header comment names
the tokens it replaces. The branch would never have expired. The phone's 1b excludes
`ThemeV5.swift` for exactly this reason; the watch scan now excludes all three token-layer
files. Worth checking on any gate that counts its own source tree.

### Known gap, pre-existing, not introduced here

The watch bundles **no font licences**. `OFL-Archivo.txt` and `OFL-InstrumentSans.txt`
are in the repo beside the TTFs but do not reach the `.app`: the watch target takes its
whole source directory as a group and xcodegen's file-type heuristic routes `.ttf` to
resources while skipping `.txt`. The phone solves this with a dedicated
`type: folder, buildPhase: resources` entry plus an exclude on the main source path.

The six fonts already shipping (Bebas / Inter / Oswald) have the same gap, so this is a
