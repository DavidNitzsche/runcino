...
Treat hex values and pixel measurements below as exact. A few interactive
affordances are intentionally left as no-ops in the prototype (shoe "Change",
"Add a pair", Settings "Sign out") — these need real handlers in the app but
their visual design is final.
...
- `Faff-iPhone-App.dc.html` — every screen listed above, self-contained,
  ...
  a measurement above is ambiguous. Two Tweaks-panel props (`verdict` on
  Races, `paceDirection` on the paces screen) switch between the content
  variants described above without editing code.
