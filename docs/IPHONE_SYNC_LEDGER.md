...
| `<pending HK fix>` | HK sleep stages · capture `HKCategoryValueSleepAnalysisAsleepUnspecified` minutes | iPhone HK reader currently sums `asleepCore + asleepREM + asleepDeep` into `sleep_hours`. Apple Health's "Time Asleep" includes `asleepUnspecified` too · the bucket HK uses when sleep is detected but stages weren't (Sleep Focus + watch off, naps, manual entries, 3rd-party apps). David's QC 2026-06-05: HK shows 7:55, Faff shows 6:47 · 66min gap = the missing unspecified bucket. Send as `sleep_unspecified_minutes` (server route whitelist already accepts it as of this commit) · iPhone reader should ALSO add it to the value sent for `sleep_hours` so the canonical total matches HK's "Time Asleep." | NEEDS WIRING |
