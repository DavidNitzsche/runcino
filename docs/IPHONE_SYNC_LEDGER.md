...
| (CI feature · pending commit, 2026-06-08) | `goalRace.confidenceInterval {lo,hi,pct,method}` + `goalRace.confidenceLabel {tier,word,descriptor,detail}` (NEW fields on the targets/projection payload) | Decode both objects. Render the CI as an explicit band around the projection (lo–hi range, e.g. "1:31:56 – 1:37:52") + the label word+descriptor ("MEDIUM · doable, not banked") on the projection header. Web reference: `TargetsView` `.bci` zone + `.goalconf`. Seed already populates both (one server number, all surfaces). See `docs/AUDIT-FIXES.md` CI-followup-2. | NEEDS WIRING · TF QUEUE |
