...
| `<pending iPhone fix>` 2026-06-05 | `POST /api/ingest/workout` · `body.splits` stopped including data between 2026-05-27 and 2026-05-29 | David QC'd 2026-06-05 — 6mi easy run shows "No mile splits available."  RO DB scan across last 14 days: `apple_watch` source rows through 2026-05-27 carry real per-mile splits arrays (5, 6, 8, 11 entries matching mileage); rows from 2026-05-29 onward carry `splits: []` + `splits_unreliable: false` + `splits_validation: null`.  Web ingest code is unchanged (validator only fires when `rawSplits.length > 0` so empty array passes through silently), proving the payload itself stopped including the splits array.  Strava connector token is stale (`last_sync_at: 2026-05-25`, 11 days idle) so no third source backfills.  Web side fixes shipped today (e9f8eafe normalizer, a10eb0f2 read-time stub guard) make the UI honest ("No mile splits available") but cannot conjure data that wasn't sent. iPhone agent needs: (1) restore splits derivation in the HK workout ingest payload — whichever 2026-05-27/28 change removed it — (2) consider also sending raw timestamped streams (`hr_samples` with tSec is already accepted for zone bucketing; adding `route_samples` with `[tSec, lat, lng]` or `distance_samples` with `[tSec, miles]` would let the web rebuild splits even if iPhone's own computation has a bug). | NEEDS IPHONE FIX |
