...
|--------|-----------------|-------------|--------|
| (this commit) 2026-06-10 | NEW `POST /api/auth/signup` { name, email, password } → same response shape as /api/auth/email (token + redirect) · multi-user opening | DONE in repo: `API.signUpWithEmail` added; `EmailSignInSheet` now has a CREATE ACCOUNT mode (Name field, mode toggle, signup-aware copy + error mapping). Also sign-out hygiene: `performSignOut` clears `faff.health.lastNightHours.v1` so the next account on the device doesn't flash the previous runner's sleep. No new decode work — `EmailSignInResponse` already carried `redirect`/`created`. **2026-06-10 PM update: faff went INVITE-ONLY same day — /api/auth/signup now answers 403 "Faff is invite-only — request access at faff.run" (the sheet surfaces it verbatim, which is honest). Future Swift pass: swap the CREATE ACCOUNT mode for a REQUEST ACCESS mode (name+email → /api/auth/request-access) + handle login redirect '/set-password' (temp-credential first sign-in; web-only screen today — iPhone temp-login lands in onboarding and keeps the temp password until the runner visits the web; acceptable until wired).** | DONE · TF QUEUE (rides build 203) · REQUEST-ACCESS MODE PENDING |
| 2026-06-10 (coached mode + coach calendar) | NEW coached mode: `profile.user_settings.coached_externally` (onboarding 'I have a coach' chip · web) + NEW `/api/coach-calendar` GET/POST/DELETE (Final Surge / any ICS Sync URL · events cached in coach_reads_cache · read-only). Web Today renders COACHED hero + FROM YOUR COACH workout per day. `/api/watch/today` already answers `{message:'No active plan.'}` for coached users — watch is honest with zero changes. | iPhone TodayView equivalent needed for coached users: render coached state (no plan prescriptions) + the coach's calendar workout for the day (read from a future seed/today payload field — web reads it via buildSeed, iPhone will need it surfaced on an API the phone already calls, e.g. /api/briefing or a new field on watch/today). Also Settings row to paste/disconnect the calendar URL. Design before build — don't improvise the surface. | NEEDS WIRING (web-only today) |
...
| ed8cdeac (2026-06-08) | `confidenceInterval {lo,hi,pct,method}` + `confidenceLabel {tier,word,descriptor,detail}` added to `/api/targets/projection` response | `ProjectionConfidenceInterval` + `ProjectionConfidenceLabel` structs added to `ToolkitPayloads.swift`; decoded on `ProjectionSummary`. `confidenceBand` view inserted in `K_TargetsProjection` between headline and meta pills: renders "1:31:56 – 1:37:52 · MEDIUM · doable, not banked". Route deployed to Railway. Swift in next TF bundle. Falsifier: iPhone Targets shows range + label matching web. | DONE · TF QUEUE |

## AUTO-RIPPLE · iPhone gets it free

| Commit | What changed | Why iPhone doesn't need to touch |
|--------|--------------|----------------------------------|
| (this commit) 2026-08-28 · biometrics pipeline | Nightly max-hr-ratchet cron now also snapshots `profile.hrmax_observed` (observed 12-mo ceiling), `profile.rhr` (7-day rolling per Research/15) and `profile.vo2max_apple` (+`_updated_at`, latest sample · display only, never pace input). `GET /api/profile` now returns `hrmax_observed` (additive field). Field-test completions (`sub_label FIELD TEST`) now auto-write `profile.lthr` (method `field_test`, Friel final-20-min avg) from watch hrSamples. Zones for a no-HR-data runner fall back to Tanaka age-predicted (loudly labeled `note` on `physiology.zones`); LTHR > measured HRmax > age precedence unchanged. | iPhone reads `ProfileState.physiology` server-rendered — max_hr/rhr/vo2/zones all resolve server-side and auto-ripple. New `hrmax_observed` GET field is additive; Swift Decodable ignores unknown keys. Zone `note` already an optional string on `ZoneTable`. LTHR update after a field test lands on the next profile/today fetch. |
...
## 2026-06-10 (later) · iPhone auth + onboarding parity SHIPPED (code)

| Change | Surface | Status |
|---|---|---|
| **Apple Sign In removed** · email/password only + "Request access" (invite-only). Backend `/api/auth/apple` gates new-account creation (existing Apple users still sign in). | SignInView/EmailSignInSheet/API.swift + apple/route.ts | Code on main (ef5b25ae). Inert SignInWithAppleView.swift left in place (delete in Xcode later — pbxproj had concurrent WIP). **Sim-build + TF still needed.** |
| **Onboarding parity** · new "Your week" step (frequency 3-6, weekly mileage, start day, long-run day); history wired; `.goal` (TT) mode fixed (was 400ing); `.coached` mode added; modes → vertical list. Sends real values (was all NSNull). | OnboardingView.swift | Code on main (90b4aa8b). **iPhone plans now honor frequency + baseline like web.** Sim-build + TF needed. |
| ⚠️ **Watch AppIcon blocks plain `xcodebuild`** | FaffWatch Assets.xcassets | PRE-EXISTING (untouched this session). Xcode 26 rejects the single 1024px watchOS AppIcon ("no applicable content"). `ship-testflight-v2.sh` presumably handles it; if a plain build is ever needed, the watchOS AppIcon needs the proper multi-slot Contents.json for Xcode 26. iPhone Swift verified-compiled regardless (object files emitted). |

## 2026-06-11 · Background HK sync — strength/workouts land without opening the app

| Change | Surface | Status |
|---|---|---|
| **Background HealthKit delivery.** `HKObserverQuery` on `workoutType()` + `enableBackgroundDelivery`, registered in `NotificationsAppDelegate.didFinishLaunchingWithOptions` (NOT a SwiftUI `.task` — only a didFinishLaunching observer runs on a background launch). iOS background-launches the app when a workout (run OR strength) lands in HK → focused `backgroundWorkoutSync()` (workouts + strength only, skips the heavy vitals sweep to stay in budget) ingests it hands-free. One observer covers every workout. | `HealthKitImporter.swift`, `NotificationsAppDelegate.swift`, `Faff.entitlements` | Code on main (**d00f02f6**). **Adds `com.apple.developer.healthkit.background-delivery` entitlement** — was missing app-wide, so ALL bg delivery (incl. HRAlerter's HR-ceiling alerts) was a silent no-op. Signs under Automatic + `-allowProvisioningUpdates`, no Developer-portal toggle. Build verified green. **QUEUE FOR NEXT TF (counter 210 → ~211).** Device-only validation: do a strength workout with Faff CLOSED, confirm it appears within ~an hour (iOS clamps workout bg delivery to ~hourly; foreground stays the instant path). |
| Strength **display** on iPhone (week-status glyph + "X/Y this week" chip; native renders none today) | native Today (TBD) | **David — via iPhone agent.** Ingest lands the data in `strength_sessions`; the display is a separate, design-led piece. Not built here. |
| 🔧 Native build-verify that beats the watch-AppIcon wall (better than the object-file trick above) | — | `xcodebuild -scheme Faff -destination 'generic/platform=iOS' CODE_SIGNING_ALLOWED=NO ASSETCATALOG_COMPILER_APPICON_NAME='' build` → real BUILD SUCCEEDED. Key: `generic/platform=iOS` (NOT `-sdk iphonesimulator`, which breaks the watchOS target on `WatchKit`) + the empty-AppIcon override. |
