...
**Update 2026-09-09T~19:50Z:** the RouteMapView root-cause fix landed (commit `2d1918225`) and a fresh independent review has been dispatched — see §0.4a, §6 row 4, §7 row 28. Two background agents remain running (recovery-ended-early wire field, status-bar-gradient review). Their rows are marked `INVESTIGATING` / `IMPLEMENTED — REVIEW PENDING` with an explicit note that this report will need one more update once they land.
...
| 4 | RouteMapView green marker | prior-session agent (`12355ae06`) + this session's root-cause fix agent `abaa7ecf0e79edbeb` (`2d1918225`) | (agent-managed) | `fix/routemap-green-start-marker` | `origin/main` | `2d1918225` | clean (pushed) | pixel-sampled `[SIM]` across 3 simulators incl. a full-erase pass | agent `ab87d2aef9f4d2602` (FAIL, on `12355ae06`); fresh review agent `a289cb75be0c0f6d3` dispatched on `2d1918225` | v1: **FAIL**; v2: **RUNNING** | NO | Wait for fresh review |
...
| 8 | Status-bar full-bleed gradient | this session, agent `ae0b497b216d37080` (`4a52e48a7`); fix v2 in progress, agent `a798f14a02260f93f` | `.claude/worktrees/agent-ae0b497b216d37080` | `fix/races-fullbleed-gradient` | `origin/main` | `4a52e48a7` | clean (pushed) | `[SIM]` before/after by implementer; reviewer's NSLog instrumentation + pixel-precise screenshots | agent `a2489596c77a30d7f` | **FAIL** — diagnosis/plumbing correct, but `DayPanel` doesn't re-layout on a later `\.v5TopInset` change; symptom unresolved | NO | Fix v2 dispatched (agent `a798f14a02260f93f`, running) |
...
| 28 | RouteMapView green start marker | **IMPLEMENTED — REVIEW PENDING** (v1 REVIEWED and FAILED; v2 root-cause fix landed, fresh review running) | Low-Medium | `[SIM]`, pixel-sampled | Rendered, defect found and then fixed on real render across 3 simulators | Low (cosmetic, but the wrong color also collides with the app's "good/on-plan" green semantics) | Low | Engineering | `fix/routemap-green-start-marker` @ `2d1918225` | v1 (`12355ae06`): **FAIL**; v2 (`2d1918225`): review running (agent `a289cb75be0c0f6d3`) | none | NO (bundled) | NOT MERGED | build only | v1 **failed** on repeated relaunch; v2 pixel-sampled correct across 3 sims incl. full-erase, awaiting independent confirmation | Wait for fresh review |
...
| 31 | Status-bar/full-bleed gradient gap (Today/Block/Races, shared root cause) | **REVIEWED — FAIL, re-fix in progress** | Medium (visible whenever offline/stale, on 3 screens) | `[TEST]`+`[SIM]`, pixel-precise | Rendered on simulator | Medium (visible whenever signal is spotty — an ordinary state, not rare) | Low | Engineering | `fix/races-fullbleed-gradient` @ `4a52e48a7` | **FAIL** — root-cause diagnosis and environment-value plumbing confirmed correct via NSLog instrumentation, but `DayPanel` (`PanelV5.swift`) does not re-layout when `\.v5TopInset` changes after first appearance, so the gradient's top edge never actually moves; the reported symptom reproduces pixel-for-pixel identically on fixed vs. unfixed code | none | YES | NOT MERGED | v1: plumbing correct, symptom unresolved | **Failed** — corrected value computed, never rendered | Fix v2 dispatched (agent `a798f14a02260f93f`, running) — must fix the SwiftUI relayout gap, not just the inset calculation |
...
| 11 | Race page | RaceDecisionCardV5 header fix reviewed PASS; RouteMapView marker fix v1 FAILED on re-render, v2 root-cause fix landed (pixel-sampled correct across 3 sims incl. full-erase), fresh review running; sample verdict-string bug found+fixed (render-verify pending); status-bar gradient fix (shared with Today/Block) implemented, review running | — | Fresh review of RouteMapView v2; render-verify of verdict-string fix | RouteMapView v1 regressed on warm relaunch — this IS a regression finding, now fixed in v2 pending confirmation | — | Wait for RouteMapView v2 review; retry verdict-string render-verify; merge header fix |
