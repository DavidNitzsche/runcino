...
| 34 | Missing pace on 5-mile easy phase + strides | **IMPLEMENTED — VERIFICATION INCOMPLETE** | High | **YES, per your explicit instruction** | `fix/postrun-missing-pace-routing` @ `a2b6715b9` | **PASS WITH CONDITIONS** | Real production-data root cause, on-device render, 496/496 + 11,90x tests | Merge conflict resolution against moved `main`, then your merge approval, build, physical verification |
| 35 | Piece-by-piece evidence hierarchy, short-stride metric, amber marker | **IMPLEMENTED — VERIFICATION INCOMPLETE** | Medium-High | **YES, per your explicit instruction** | Same branch as #34 | Same review as #34 | Confirmed: `.modelled` flag reused correctly (no new amber mark needed — app-wide tilde retirement confirmed as your own 2026-08-21 ruling), "Pace unavailable" state confirmed on-device | Same as #34 |
